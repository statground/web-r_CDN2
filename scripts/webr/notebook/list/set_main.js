function set_main() {
	...
	function SkeletonRow() {
		return (
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full py-3 border-b border-slate-100 px-2">
	...
	function NotebookRow({ nb, onToggleFavorite, onCopyUrl, copied, onDelete }) {
		return (
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full py-3 border-b border-slate-100 px-2 hover:bg-slate-50">
				<div className="flex items-start gap-2 w-full sm:w-auto">
					<button
						className={
							"mt-0.5 text-sm " +
	...
	function Div_main() {
		...
		async function fetchNotebookList({ nextOffset, append }) {
			...
			try {
				...
				form.append("visibility", visibilityFilter); // all/public/private
				form.append("limit", String(PAGE_SIZE));
				form.append("offset", String(nextOffset));

				const res = await fetch("{% url 'webr:ajax_get_notebook_list' %}", {
					method: "POST",
					body: form,
				});
				...
				const mapped = items.map((r, idx) => {
					...
					const isPublic = (r.share === 1 || r.share === true);
		...
		/****************************************************************
		 * ✅ favoriate 토글: 서버 반영 후 1페이지 리로드(정렬 서버 기준)
		 ****************************************************************/
		const toggleFavorite = async (id) => {
			try {
				...
				const res = await fetch("{% url 'webr:ajax_toggle_notebook_favoriate' %}", {
					method: "POST",
					body: form,
				});
				...
				// ✅ 서버 정렬 기준이므로 1페이지 리로드(최상단 이동 포함)
				fetchNotebookList({ nextOffset: 0, append: false });
		...
		const copyPublicUrl = async (id) => {
			...
			const origin =
				window.location && window.location.origin
					...
					: "";
			const url = `${origin}/webr/notebook/run/${target.uuidShare}/`;
		...
		const deleteNotebook = async (id) => {
			...
			try {
				...
				const res = await fetch("{% url 'webr:ajax_delete_notebook' %}", {
					method: "POST",
					body: form,
				});
		...
		return (
			<div className="flex flex-col justify-center items-center py-8 px-4 w-full max-w-screen-sm mx-auto md:px-8">
