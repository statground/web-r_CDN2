...
async function apiLogin(email, password) {
  ...
  if (!res.ok) return { result: "fail", err: "login_error_" + res.status, raw: data };

  // server: {"checker":"DONE"|"NOTEXIST"|"WRONGPASSWORD"}
  if (data && (data.checker === "DONE" || data.checker === "SUCCESS")) return { result: "ok", raw: data };
  if (data && data.checker === "WRONGPASSWORD") return { result: "fail", err: "비밀번호가 올바르지 않습니다.", raw: data };
  if (data && data.checker === "NOTEXIST") return { result: "fail", err: "계정을 찾을 수 없습니다.", raw: data };
...
async function apiSignup(payload) {
  ...
  // server sometimes returns plain text "SUCCESS"
  const text = await res.text().catch(() => "");
  if (!res.ok) return { result: "fail", err: "signup_error_" + res.status, raw: text };

  try {
    const data = JSON.parse(text);
    if (data && (data.checker === "SUCCESS" || data.result === "ok")) return { result: "ok", raw: data };
    return { result: "fail", err: (data && (data.err || data.msg)) ? (data.err || data.msg) : "회원가입에 실패했습니다.", raw: data };
  } catch (e) {
    if ((text || "").toUpperCase().indexOf("SUCCESS") >= 0) return { result: "ok", raw: text };
...
async function apiSendAuthEmail(email) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) return { result: "fail", err: "mail_error_" + res.status, raw: data };

  // server: {"exist":"EXIST"|"NOTEXIST","code":"...."}
  if (data && data.exist === "NOTEXIST") return { result: "fail", err: "가입한 이메일이 아닙니다.", raw: data };
  if (data && data.code) return { result: "ok", raw: data };

  // If email was sent but server response shape differs, treat 200 as ok
