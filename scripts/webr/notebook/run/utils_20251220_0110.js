...
/**
 * 회원가입
 * - account 앱의 ajax_signup에 맞춰 FormData로 전송합니다.
 */
async function apiSignup(params) {
  ...
  const nickname = (params && params.nickname) ? params.nickname : "";
  const realname = (params && params.realname) ? params.realname : "";
  const gender = (params && params.gender) ? params.gender : "";

  const fd = new FormData();
  fd.append("txt_email", email);
  fd.append("txt_password", password);
  fd.append("txt_name", nickname);
  fd.append("txt_realname", realname);
  fd.append("sel_gender", gender);
...
/**
 * 비밀번호 찾기: 인증 메일 발송 (GET param email)
 */
