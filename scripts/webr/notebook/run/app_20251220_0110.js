...
function Notebook() {
  ...
  const [shareMode, setShareMode] = useState(0); // 0: OFF, 1: community, 2: link-only
  ...
  // =========================
  // Runtime Info (R version, platform)
  // - Filled after WebR is initialized by running sessionInfo()
  // =========================
...
useEffect(() => {
  ...
  const [loginTab, setLoginTab] = useState("signin"); // signin | signup | forgot
  ...
  const [suRealname, setSuRealname] = useState("");
  ...
  const [fpStep, setFpStep] = useState(1); // 1: send mail, 2: verify, 3: reset
  ...
  async function refreshAuth() {
    ...
    try {
      ...
      // 네트워크/일시 오류로 인한 '로그아웃처럼 보이는 현상' 방지
      if (authFailCountRef.current >= 3) setAuthUser(null);
    } finally {
...
function showToast(message, kind = "ok") {
  ...
  async function fetchRuntimeSessionInfo(webr) {
    try {
      ...
      // Prefer evalRString if available (cleanest).
      try {
  ...
  useEffect(() => {
    ...
    async function initWebR() {
      try {
        ...
        // Populate runtime info panel (sessionInfo)
...
const payload = {
  ...
  // =========================
  // Auto-save (every 60s) while RUN page is open
  // - Only in edit mode and when runtime is ready
  // - Uses _skip_confirm + autosave flag (backend can treat it specially)
  // =========================
  useEffect(() => {
    if (!canEdit) return;
    ...
    const timer = setInterval(async () => {
      try {
        ...
        // silent fail (avoid disturbing the user every minute)
        console.warn("autosave failed:", e);
      }
    }, 60 * 1000);

    return () => clearInterval(timer);
  }, [canEdit, isViewMode, notebookTitle, cells, shareMode, darkMode, runtimeSessionInfo]);
  ...
  async function handleSetShareMode(nextMode) {
    ...
    try {
      ...
      console.log("[Share] set mode click", { nextMode, nbid });
          ...
          <div className="flex w-full flex-wrap items-center gap-2 text-[12px] lg:w-auto lg:justify-end">
            ...
            <div className="hidden lg:flex items-center gap-2 ml-2">
              {authLoading ? (
                ...
                <div className="flex items-center gap-2">
                  <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] dark:border-slate-700 dark:bg-slate-950">
                    <span className="text-orange-500">●</span>
                    <div className="leading-tight">
                      <div className="font-semibold text-slate-800 dark:text-slate-100">
                        {authUser.nickname || authUser.name || (authUser.email ? authUser.email.split("@")[0] : "User")}
...
>
  ...
  {/* Package Manager */}
          ...
          <section className="mb-2">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="text-[12px] font-semibold text-slate-600 dark:text-slate-200">RUNTIME (sessionInfo)</h2>
              {!isViewMode && (
                <button
                  type="button"
                  onClick={() => { if (webrInstance) fetchRuntimeSessionInfo(webrInstance); }}
            ...
            <div className={"rounded-xl border text-[11px] " + (darkMode ? "border-slate-800 bg-slate-900" : "border-slate-200 bg-white")}>
              <div className="border-b px-2 py-1.5 text-[11px] text-slate-400 dark:border-slate-800">
                {isViewMode ? "View mode" : (ready ? "WebR ready" : "Starting...")}
              </div>

              {runtimeSessionInfoError ? (
                <div className="px-2 py-2 text-[11px] text-rose-500">{runtimeSessionInfoError}</div>
              ) : null}

              <pre className={"px-2 py-2 whitespace-pre-wrap break-words font-mono " + (darkMode ? "text-slate-100" : "text-slate-800")}>
                {runtimeSessionInfo || (isViewMode ? "Runtime not started in view mode." : "Loading...")}
              </pre>
            </div>
          </section>
            ...
            <div className="space-y-2 text-[12px]">
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <button
                    ...
                    className={"rounded-full px-3 py-1 text-xs font-medium text-white " + (shareMode === 0 ? "bg-slate-700" : "bg-slate-400 hover:bg-slate-500")}
                  >
                    OFF
                  </button>
                  <button
                    ...
                    className={"rounded-full px-3 py-1 text-xs font-medium text-white " + (shareMode === 1 ? "bg-emerald-600 hover:bg-emerald-700" : "bg-slate-600 hover:bg-slate-700")}
                  >
                    커뮤니티에 공유
                  </button>
                  <button
                    ...
                    className={"rounded-full px-3 py-1 text-xs font-medium text-white " + (shareMode === 2 ? "bg-indigo-600 hover:bg-indigo-700" : "bg-slate-600 hover:bg-slate-700")}
                  >
                    링크로만 공유
                  </button>
                </div>
              ...
              {shareEnabled && shareUUID ? (
                <div className={"rounded-xl border px-3 py-2 text-[11px] " + (darkMode ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-slate-50")}>
                  ...
                  <div className="mb-2 text-[11px] text-slate-400">{shareMode === 1 ? "공개 범위: 커뮤니티" : shareMode === 2 ? "공개 범위: 링크 보유자" : ""}</div>
...
{showLogin && (
  ...
  >
    <div className={"w-full max-w-sm rounded-2xl border p-4 shadow-2xl " + (darkMode ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-200 bg-white text-slate-800")}>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-sm font-semibold">
          {loginTab === "signin" ? "Web-R 로그인" : (loginTab === "signup" ? "Web-R 회원가입" : "비밀번호 찾기")}
        </h2>
      ...
      <div className="mb-3 flex gap-2 text-[11px]">
        <button
          type="button"
          onClick={() => { setAuthError(""); setLoginTab("signin"); }}
          className={"flex-1 rounded-full border px-3 py-1 " + (loginTab === "signin" ? (darkMode ? "border-orange-500 bg-orange-500/10 text-orange-200" : "border-orange-400 bg-orange-50 text-orange-700") : (darkMode ? "border-slate-700" : "border-slate-200"))}
        >
          로그인
        </button>
        <button
          type="button"
          onClick={() => { setAuthError(""); setLoginTab("signup"); }}
          className={"flex-1 rounded-full border px-3 py-1 " + (loginTab === "signup" ? (darkMode ? "border-orange-500 bg-orange-500/10 text-orange-200" : "border-orange-400 bg-orange-50 text-orange-700") : (darkMode ? "border-slate-700" : "border-slate-200"))}
        >
          회원가입
        </button>
        <button
          type="button"
          onClick={() => { setAuthError(""); setLoginTab("forgot"); setFpStep(1); }}
          className={"flex-1 rounded-full border px-3 py-1 " + (loginTab === "forgot" ? (darkMode ? "border-orange-500 bg-orange-500/10 text-orange-200" : "border-orange-400 bg-orange-50 text-orange-700") : (darkMode ? "border-slate-700" : "border-slate-200"))}
        >
          비번찾기
        </button>
      </div>
      ...
      <p className="mb-3 text-[11px] text-slate-400">
        Web-R 홈페이지 계정과 로그인/로그아웃 상태를 연동합니다.
      </p>
      ...
      {loginTab === "signin" && (
        <form className="space-y-3 text-[12px]">
          <div className="space-y-1">
            ...
            <input type="text" value={loginAccount} onChange={(e) => setLoginAccount(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} placeholder="example@domain.com" />
          </div>
          <div className="space-y-1">
            <label className="block text-[11px] text-slate-500">Password</label>
            <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} placeholder="••••••••" />
          </div>

          <div className="mt-2 flex justify-end gap-2">
            <button type="button" onClick={() => { setAuthError(""); setShowLogin(false); }} className="rounded-full border px-3 py-1">취소</button>
            <button
              type="button"
              disabled={authWorking}
              className={"rounded-full px-3 py-1 " + (darkMode ? "bg-orange-500/20 text-orange-100 border border-orange-400/40" : "bg-orange-500 text-white")}
              onClick={async () => {
                setAuthError("");
                const email = (loginAccount || "").trim();
                const pass = loginPassword || "";
                if (!email || !pass) { setAuthError("Email/Password를 입력해 주세요."); return; }
                setAuthWorking(true);
                try {
                  ...
                  setAuthError(String((e && e.message) ? e.message : e));
                } finally {
                  setAuthWorking(false);
                }
              }}
      ...
      {loginTab === "signup" && (
        <form className="space-y-3 text-[12px]">
          <div className="space-y-1">
            <label className="block text-[11px] text-slate-500">Email</label>
            <input type="text" value={suEmail} onChange={(e) => setSuEmail(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="block text-[11px] text-slate-500">Password</label>
              <input type="password" value={suPassword} onChange={(e) => setSuPassword(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
            </div>
            <div className="space-y-1">
              <label className="block text-[11px] text-slate-500">Confirm</label>
              <input type="password" value={suPassword2} onChange={(e) => setSuPassword2(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="block text-[11px] text-slate-500">Nickname</label>
              <input type="text" value={suNickname} onChange={(e) => setSuNickname(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
            </div>
            <div className="space-y-1">
              <label className="block text-[11px] text-slate-500">Real name</label>
              <input type="text" value={suRealname} onChange={(e) => setSuRealname(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
            </div>
          </div>
          <div className="space-y-1">
            <label className="block text-[11px] text-slate-500">Gender</label>
            <select value={suGender} onChange={(e) => setSuGender(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")}>
          ...
          <div className="mt-2 flex justify-end gap-2">
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!email) { setAuthError("Email을 입력해 주세요."); return; }
                if (!suPassword) { setAuthError("Password를 입력해 주세요."); return; }
                if (suPassword !== suPassword2) { setAuthError("Password가 일치하지 않습니다."); return; }
                if (!suNickname) { setAuthError("Nickname을 입력해 주세요."); return; }
                if (!suRealname) { setAuthError("Real name을 입력해 주세요."); return; }
                if (!suGender) { setAuthError("Gender를 선택해 주세요."); return; }

                setAuthWorking(true);
                try {
                  await apiSignup({ email, password: suPassword, nickname: suNickname, realname: suRealname, gender: suGender });
                  await refreshAuth();
                  setShowLogin(false);
                } catch (e) {
                  setAuthError(String((e && e.message) ? e.message : e));
        ...
        <form className="space-y-3 text-[12px]">
          {fpStep === 1 && (
            <>
              <div className="space-y-1">
                <label className="block text-[11px] text-slate-500">Email</label>
                <input type="text" value={fpEmail} onChange={(e) => setFpEmail(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
              </div>
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => { setAuthError(""); setShowLogin(false); }} className="rounded-full border px-3 py-1">닫기</button>
                <button
                  type="button"
                  disabled={authWorking}
                  className={"rounded-full px-3 py-1 " + (darkMode ? "bg-orange-500/20 text-orange-100 border border-orange-400/40" : "bg-orange-500 text-white")}
          ...
          {fpStep === 2 && (
            <>
              <div className="space-y-1">
                <label className="block text-[11px] text-slate-500">인증코드</label>
                <input type="text" value={fpCode} onChange={(e) => setFpCode(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} placeholder="메일로 받은 코드" />
              </div>
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setFpStep(1)} className="rounded-full border px-3 py-1">이전</button>
                <button
                  type="button"
                  disabled={authWorking}
                  className={"rounded-full px-3 py-1 " + (darkMode ? "bg-orange-500/20 text-orange-100 border border-orange-400/40" : "bg-orange-500 text-white")}
                  onClick={async () => {
                    setAuthError("");
                    if (!fpCode) { setAuthError("인증코드를 입력해 주세요."); return; }
                    setAuthWorking(true);
                    try {
                      await apiCheckAuthCode(fpCode);
                      setFpStep(3);
                    } catch (e) {
                      setAuthError(String((e && e.message) ? e.message : e));
                    } finally {
          ...
          {fpStep === 3 && (
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="block text-[11px] text-slate-500">New password</label>
                  <input type="password" value={fpNewPass} onChange={(e) => setFpNewPass(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] text-slate-500">Confirm</label>
                  <input type="password" value={fpNewPass2} onChange={(e) => setFpNewPass2(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950/40 text-slate-100" : "border-slate-200 bg-white")} />
                </div>
              </div>
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setFpStep(2)} className="rounded-full border px-3 py-1">이전</button>
                <button
                  type="button"
                  disabled={authWorking}
                  className={"rounded-full px-3 py-1 " + (darkMode ? "bg-orange-500/20 text-orange-100 border border-orange-400/40" : "bg-orange-500 text-white")}
                  onClick={async () => {
                    ...
                    const email = (fpEmail || "").trim();
                    if (!email) { setAuthError("Email을 입력해 주세요."); return; }
                    if (!fpNewPass) { setAuthError("새 비밀번호를 입력해 주세요."); return; }
                    if (fpNewPass !== fpNewPass2) { setAuthError("새 비밀번호가 일치하지 않습니다."); return; }
                    setAuthWorking(true);
                    try {
                      await apiPasswordChange(email, fpNewPass);
                      // 비밀번호 변경 후 서버가 로그아웃 처리할 수 있으니 상태 갱신
                      await refreshAuth();
                      setShowLogin(false);
                    } catch (e) {
                      setAuthError(String((e && e.message) ? e.message : e));
