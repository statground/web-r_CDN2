async function get_app_list() {
	let tag = ""
	if (url == "None") {tag = "Free"}
	if (url == "member") {tag = "Advance"}
	...
	function Div_app_list(props) {
		const app_list = Object.keys(props.data).map((btn_data) =>  
			<div class="flex flex-col justify-center items-center bg-white rounded-lg border border-gray-200 shadow-sm p-5 space-y-2">
				...
				{
					gv_username != "" && props.data[btn_data].auth == "NO"
					?   <p class="text-sm text-red-500">정회원 이상만 접속할 수 있습니다.</p>
					:   ""
				}
				{
					gv_username != "" && props.data[btn_data].auth == "YES"
