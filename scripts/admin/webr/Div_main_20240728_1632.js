				...
				<div class="w-full bg-white border border-gray-200 rounded-lg shadow">
					<div class="p-4 bg-white rounded-lg md:p-8 text-center">
						...
						<dl class="grid grid-cols-4 w-full md:grid-cols-2 gap-8 p-4 mx-auto text-gray-900 md:p-8">
							<Div_sub_card title={"총 접속 횟수"} value={props.data.count.val_connect_app_total['0']} unit={"회"}
										  subtitle={"총 이용자 수"} subvalue={props.data.count.val_user_total['0']} subunit={"명"}/>
							<Div_sub_card title={"올해 접속 횟수"} value={props.data.count.val_connect_app_yearly['0']} unit={"회"}
										  subtitle={"올해 이용자 수"} subvalue={props.data.count.val_user_yearly['0']} subunit={"명"}/>
							<Div_sub_card title={"이번 달 접속 횟수"} value={props.data.count.val_connect_app_monthly['0']} unit={"회"}
										  subtitle={"이번 달 이용자 수"} subvalue={props.data.count.val_user_monthly['0']} subunit={"명"}/>
							<Div_sub_card title={"오늘 접속 횟수"} value={props.data.count.val_connect_app_daily['0']} unit={"회"}
										  subtitle={"오늘 이용자 수"} subvalue={props.data.count.val_user_daily['0']} subunit={"명"}/>
						</dl>
					</div>
				</div>
