async function get_book_list() {
	...
	function Div_book_list({data}) {
		const renderBookCard = (btn_data) => {
			const handleClick = () => {
				// Hide any existing purchase buttons
				document.querySelectorAll('.purchase-buttons').forEach(btn => {
					btn.style.display = 'none';
				});
				
				// Show purchase buttons for clicked card
				const purchaseBtns = document.getElementById(`purchase-btns-${btn_data}`);
				if (purchaseBtns) {
					purchaseBtns.style.display = 'flex';
				}
