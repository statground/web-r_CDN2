...
function Div_main(props) {
  ...
  const list_app_legacy = list_app.filter((x) => (x.app_group || "\uAE30\uC874 \uC571") === "\uAE30\uC874 \uC571");
  const list_app_webr2 = list_app.filter((x) => (x.app_group || "\uAE30\uC874 \uC571") === "Web-R 2.0");
  const appGroupRows = Object.values(data.list_app_group || {});
  const appGroupMap = appGroupRows.reduce((acc, row) => {
    acc[row.app_group || "\uAE30\uC874 \uC571"] = row;
    return acc;
  }, {});
  const legacySummary = appGroupMap["\uAE30\uC874 \uC571"] || { cnt_connect: 0, cnt_user: 0 };
...
function draw_chart(inputData, activeTabId, groupedData) {
  ...
  const groupByNameDate = groupRows.reduce((acc, row) => {
    const group = row.app_group || "\uAE30\uC874 \uC571";
    if (!acc[group])
      acc[group] = {};
  ...
  const legacyConnects = categories.map((date) => groupByNameDate["\uAE30\uC874 \uC571"] && groupByNameDate["\uAE30\uC874 \uC571"][date] ? groupByNameDate["\uAE30\uC874 \uC571"][date].cnt_connect || 0 : 0);
  ...
  const legend = hasGroupedRows ? ["\uAE30\uC874 \uC571 \uC811\uC18D", "Web-R 2.0 \uC811\uC18D", "\uC804\uCCB4 \uC774\uC6A9\uC790 \uC218"] : ["\uC811\uC18D \uD69F\uC218", "\uC774\uC6A9\uC790 \uC218"];
  const series = hasGroupedRows ? [
    { name: "\uAE30\uC874 \uC571 \uC811\uC18D", type: "bar", stack: "connect", yAxisIndex: 0, data: legacyConnects, barMaxWidth: 24 },
  ...
  const option = {
    title: { text: "\uAE30\uC874 \uC571 / Web-R 2.0 \uC774\uC6A9 \uCD94\uC774 \uADF8\uB798\uD504", left: "center", top: 0, textStyle: { fontSize: 24, fontWeight: "700" } },
