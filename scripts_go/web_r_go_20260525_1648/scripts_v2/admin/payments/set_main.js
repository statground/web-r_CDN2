...
function Div_payer_table(props) {
  ...
  const countText = loading ? "검색 중" : rows.length.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "건";
  const tableRows = visibleRows.length > 0 ? visibleRows.map((row, index) => {
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement(
      ...
      rows.length > visibleRows.length ? /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-right text-xs text-gray-500" }, "상위 ", visibleRows.length, "건 표시") : null
    )
  );
}
