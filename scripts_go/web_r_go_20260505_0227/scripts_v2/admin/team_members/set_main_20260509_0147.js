...
const TEAM_ADMIN_SCOPES = {
  ...
  admin_team: {
    title: "관리자 팀",
    loading: "관리자 팀 현황을 불러오고 있습니다.",
    empty: "관리자 팀에 해당하는 계정이 없습니다.",
    searchPlaceholder: "관리자 팀 계정 검색",
    teamKinds: ["internal_admin"]
  },
  tester_team: {
    title: "테스터 팀",
    loading: "테스터 팀 현황을 불러오고 있습니다.",
    empty: "테스터 팀에 해당하는 계정이 없습니다.",
    searchPlaceholder: "테스터 팀 계정 검색",
...
function Div_operation_menu() {
  ...
  return React.createElement("div", { className: "col-span-2 md:grid-cols-1 justify-center item-center" },
    React.createElement("div", { className: "flex flex-col md:flex-row lg:w-48 md:w-full item-center" },
      ...
      React.createElement(Div_menu_button, { name: "정산액 조회", url: "/admin/balance_account/?year=" + date.getFullYear().toString() + "&month=" + (date.getMonth() + 1).toString() })
    )
  );
}
...
function TeamMembersApp(props) {
  ...
  const displaySummary = Array.isArray(scope.teamKinds) ? scopedSummary : summary;
        ...
        React.createElement("div", { className: "w-full rounded-lg border border-gray-200 bg-white shadow" },
          React.createElement("div", { className: "p-4 text-center md:p-8" },
            ...
            React.createElement("dl", { className: "grid grid-cols-5 w-full md:grid-cols-1 gap-8 p-4 mx-auto text-gray-900 md:p-8" },
              React.createElement(TeamMetric, { title: "팀 수", value: teamAdminFormatNumber(displaySummary.team_count) + "팀" }),
              React.createElement(TeamMetric, { title: "정회원 팀", value: teamAdminFormatNumber(displaySummary.regular_team_total) + "팀" }),
              React.createElement(TeamMetric, { title: "VIP회원 팀", value: teamAdminFormatNumber(displaySummary.vip_team_total) + "팀" }),
              React.createElement(TeamMetric, { title: "내부팀", value: teamAdminFormatNumber(displaySummary.internal_team_total) + "팀" }),
              React.createElement(TeamMetric, { title: "총 좌석", value: teamAdminFormatNumber(displaySummary.seat_limit_total) + "석" }),
              React.createElement(TeamMetric, { title: "사용 좌석", value: teamAdminFormatNumber(displaySummary.seat_used_total) + "석" }),
              React.createElement(TeamMetric, { title: "잔여 좌석", value: teamAdminFormatNumber(displaySummary.remaining_total) + "석" }),
              React.createElement(TeamMetric, { title: "결제 합계", value: teamAdminMoney(displaySummary.ledger_amount_total) })
            )
          )
        ),
...
async function set_main() {
  ...
  try {
    const headerData = await fetch("/ajax_get_menu_header/", { credentials: "same-origin" }).then(function (res) { return res.json(); });
    ...
    const data = await fetch("/admin/ajax_get_admin_team_members/", { credentials: "same-origin" }).then(function (res) { return res.json(); });
    if (data && data.ok === false) {
