async function set_main() {
  ...
  if (gv_username != "") {
    ReactDOM.render(/* @__PURE__ */ React.createElement(Div_check_admin, null), document.getElementById("div_main"));
    const data = await fetch("/ajax_get_menu_header/", { method: "POST" }).then((res) => {
      return res.json();
    }).then((res) => {
      return res;
    });
