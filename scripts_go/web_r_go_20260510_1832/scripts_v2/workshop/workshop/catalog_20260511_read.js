const WorkshopCatalogPage = (() => {
  ...
  function WorkshopDetail({ item }) {
    ...
    return h("div", { className: "w-full" }, h("section", { className: "mx-auto grid w-full max-w-screen-xl grid-cols-1 gap-8 px-6 py-10 lg:grid-cols-2" }, h("div", { className: "order-2 lg:order-1" }, h("a", { href: "/workshop/", className: "mb-6 inline-flex items-center text-sm font-semibold text-blue-700 hover:text-blue-900" }, "\uBAA9\uB85D\uC73C\uB85C"), h("div", { className: "mb-4 flex flex-row flex-wrap gap-2" }, h("span", { className: "bg-blue-600 px-3 py-1 text-xs font-semibold text-white" }, statusLabel(item.status)), h("span", { className: priceKind(item) === "free" ? "bg-green-50 px-3 py-1 text-xs font-semibold text-green-700" : "bg-red-50 px-3 py-1 text-xs font-semibold text-red-700" }, priceLabel(item))), h("h1", { className: "text-4xl font-extrabold leading-tight text-gray-900 sm:text-3xl" }, item.title), item.subtitle && h("p", { className: "mt-4 text-xl font-semibold text-gray-700 sm:text-lg" }, item.subtitle), item.summary && h("p", { className: "mt-5 text-base leading-7 text-gray-600" }, item.summary), h("div", { className: "mt-8 grid grid-cols-1 gap-x-6 md:grid-cols-2" }, h(DetailStat, { label: "\uC77C\uC815", value: displayDateRange(item) }), h(DetailStat, { label: "\uC7A5\uC18C", value: item.venue || "\uC628\uB77C\uC778/\uBCC4\uB3C4 \uC548\uB0B4" }), h(DetailStat, { label: "\uC815\uC6D0", value: item.capacity ? `${money(item.capacity)}\uBA85` : "\uBCC4\uB3C4 \uC548\uB0B4" }), h(DetailStat, { label: "\uB4F1\uB85D \uBC29\uC2DD", value: registrationLabel(item.registration_mode) }))), h("div", { className: "order-1 lg:order-2" }, imageURL ? h("img", { className: "aspect-[16/10] w-full border border-gray-200 object-cover", src: imageURL, alt: item.title, loading: "lazy" }) : h("div", { className: "flex aspect-[16/10] w-full flex-col items-center justify-center border border-gray-200 bg-gray-50 text-center" }, h("img", { className: "mb-4 h-20 w-20 opacity-80", src: "https://cdn.jsdelivr.net/gh/statground/web-R_CDN@f3e464e95616fa13712baa6adbbb0b6cda7ee821/images/svg/R_logo.svg", alt: "", loading: "lazy" }), h("p", { className: "px-6 text-xl font-extrabold text-gray-900" }, "Web-R Workshop")))), h("section", { className: "mx-auto grid w-full max-w-screen-xl grid-cols-1 gap-8 px-6 pb-12 lg:grid-cols-[minmax(0,1fr)_360px]" }, h("article", { className: "border-y border-gray-200 py-8" }, h("h2", { className: "text-2xl font-extrabold text-gray-900" }, "\uC0C1\uC138 \uB0B4\uC6A9"), descriptionLines.length ? h("div", { className: "mt-5 space-y-4 text-base leading-8 text-gray-700" }, descriptionLines.map((line, index) => h("p", { key: index }, line))) : h("p", { className: "mt-5 text-base leading-8 text-gray-500" }, "\uC0C1\uC138 \uB0B4\uC6A9\uC740 \uC900\uBE44 \uC911\uC785\uB2C8\uB2E4.")), h("aside", { className: "border-y border-gray-200 py-8" }, h("h2", { className: "text-xl font-extrabold text-gray-900" }, "\uC2E0\uCCAD \uC815\uBCF4"), h("p", { className: "mt-3 text-sm leading-6 text-gray-600" }, registrationLabel(item.registration_mode)), h("div", { className: "mt-5 space-y-3" }, hasMemberPlan && h(ProductPlan, { label: "\uD68C\uC6D0", title: item.member_product_title, price: item.member_price }), hasNonmemberPlan && h(ProductPlan, { label: "\uBE44\uD68C\uC6D0", title: item.nonmember_product_title, price: item.nonmember_price }), !hasMemberPlan && !hasNonmemberPlan && h(ProductPlan, { label: "\uCC38\uAC00\uBE44", title: priceLabel(item), price: priceKind(item) === "free" ? 0 : item.member_price || item.nonmember_price })), h("div", { className: "mt-6 border-t border-gray-100 pt-4 text-sm text-gray-500" }, item.paid_count ? `${money(item.paid_count)}\uBA85\uC774 \uC2E0\uCCAD\uD588\uC2B5\uB2C8\uB2E4.` : "\uC2E0\uCCAD \uD604\uD669\uC740 \uC900\uBE44 \uC911\uC785\uB2C8\uB2E4."), state.isAdmin && h("a", { className: "mt-5 inline-flex w-full justify-center rounded border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50", href: editHref(item) }, "\uC6CC\uD06C\uC0F5 \uC218\uC815"))));
  }
  function ReadPage() {
