const WorkshopCatalogPage = (() => {
  ...
  const conferenceFallbackImage = currentCDN2Base() ? currentCDN2Base() + "images/banner/r_conference_fallback_20260526.svg" : "";
  ...
  const statusBadgeBase = "inline-flex h-[24px] min-w-[44px] items-center justify-center rounded-md px-2 text-[12px] font-extrabold leading-none";
  ...
  function StatusBadge(props) {
    return h("span", { className: statusBadgeBase + " " + (statusBadgeTones[props.tone] || statusBadgeTones.source) }, props.label);
  }
  ...
  function WorkshopVisual({ item }) {
    const imageURL = displayImageURL(item.cover_image_url) || (item.external ? conferenceFallbackImage : "");
    return /* @__PURE__ */ React.createElement("div", { className: "relative flex h-72 w-full items-center justify-center overflow-hidden bg-gray-100" }, imageURL ? /* @__PURE__ */ React.createElement(
      "img",
      {
        ...
        onError: (event) => {
          ...
          if (conferenceFallbackImage && node.src !== conferenceFallbackImage) {
            node.src = conferenceFallbackImage;
            return;
          }
