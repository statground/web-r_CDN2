(function () {
  const STYLE_ID = "webr-admin-left-tabs-style-20260510-0525";
  const ACCORDION_VERSION = "20260510_0525";
  const MENU_ITEMS = [
  ...
  function nativeMenuButton(item, activeKey) {
    const selected = item.key === activeKey;
    return React.createElement("button", {
      key: item.key,
      type: "button",
      className: "webr-admin-tab" + (selected ? " webr-admin-tab-active" : ""),
  ...
  function AdminOperationMenu() {
    const activeKey = currentKey();
    const mainItem = menuItemForKey("main");
    return React.createElement("nav", {
      className: "col-span-2 md:grid-cols-1 justify-center item-center webr-admin-menu",
      "aria-label": "관리자 메뉴"
    },
      React.createElement("div", {
        className: "webr-admin-menu-list",
        "data-webr-admin-accordion-version": ACCORDION_VERSION,
        "data-webr-admin-active-key": activeKey
      },
        React.createElement("div", { className: "webr-admin-menu-home" }, nativeMenuButton(mainItem, activeKey)),
        MENU_GROUPS.map(function (group) {
          return React.createElement("details", {
            key: group.key,
            className: "webr-admin-accordion",
            open: groupContainsActive(group, activeKey) || activeKey === "main"
          },
            React.createElement("summary", { className: "webr-admin-accordion-summary" }, group.label),
            React.createElement("div", { className: "webr-admin-accordion-panel" },
              group.items.map(function (key) {
                const item = menuItemForKey(key);
                return item ? nativeMenuButton(item, activeKey) : null;
              })
            )
          );
        })
  ...
  function patchAdminMenu() {
    ...
    if (!menu.classList.contains("webr-admin-menu")) {
      menu.classList.add("webr-admin-menu");
    }
    buttons.forEach(function (button) {
      const item = menuItemForLabel(textOf(button));
