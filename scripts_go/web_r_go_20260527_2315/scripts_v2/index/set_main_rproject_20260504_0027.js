...
function homeDateValue(item) {
  const keys = ["updated_at", "created_at", "published_at", "pub_date", "source_updated_at", "source_collected_at", "starts_at", "event_at", "sort_key", "date"];
...
function Div_home_update_dashboard() {
  ...
  React.useEffect(() => {
    ...
    homePostJSON("/ajax_index_statistics/").then((statistics) => {
      finishSlice("statistics", { statistics: statistics || {} });
    });
    return () => {
      mounted = false;
    };
  ...
  function feedItem(lane, source, row, title, meta, href, detail, sortValue, image, isNew) {
    return {
      lane,
      source,
      row,
  ...
  homeState.workshops.slice(0, 4).forEach((row) => {
    ...
    feed.push(feedItem("workshops", "워크샵", row, preview.title, preview.meta, preview.href, row.summary || row.description || "", homeDateValue({ starts_at: row.starts_at, created_at: row.created_at }), preview.image, homeExplicitNew(row)));
  });
  const dedupedFeed = [];
  ...
  const categories = categoryDefs.map((section) => {
  ...
  function YoutubePanel() {
    ...
    const image = randomYoutube ? randomYoutube.youtube_thumbnail || randomYoutube.thumbnail_url || randomYoutube.thumbnail || randomYoutube.image || randomYoutube.url_image || "" : "";
    return /* @__PURE__ */ React.createElement("aside", { class: "overflow-hidden rounded-lg border border-slate-100 bg-slate-50/80 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { class: "flex items-center justify-between gap-3 px-4 py-3" }, /* @__PURE__ */ React.createElement("h3", { class: "text-base font-extrabold text-slate-950" }, "YouTube"), /* @__PURE__ */ React.createElement("a", { href: "/workshop/youtube/", class: "text-xs font-extrabold text-blue-700 hover:text-blue-900" }, "더 보기")), homeState.loading.youtube && !randomYoutube ? /* @__PURE__ */ React.createElement("div", { class: "px-4 pb-4" }, /* @__PURE__ */ React.createElement("div", { class: "aspect-video rounded-md bg-white animate-pulse" }), /* @__PURE__ */ React.createElement("div", { class: "mt-3 h-3 w-4/5 rounded-full bg-slate-300 animate-pulse" }), /* @__PURE__ */ React.createElement("div", { class: "mt-2 h-2 w-1/3 rounded-full bg-slate-200 animate-pulse" })) : randomYoutube ? /* @__PURE__ */ React.createElement("a", { href, class: "block px-4 pb-4 hover:bg-white/70" }, image ? /* @__PURE__ */ React.createElement("img", { src: image, alt: "", class: "aspect-video w-full rounded-md border border-slate-200 bg-white object-cover", loading: "lazy", onError: (event) => {
      event.currentTarget.style.display = "none";
    } }) : /* @__PURE__ */ React.createElement("div", { class: "flex aspect-video w-full items-center justify-center rounded-md border border-slate-200 bg-slate-100 text-sm font-extrabold text-slate-500" }, "YouTube"), /* @__PURE__ */ React.createElement("span", { class: "mt-3 flex flex-wrap items-center gap-2" }, /* @__PURE__ */ React.createElement("span", { class: "text-sm font-extrabold leading-5 text-slate-950" }, homeShortText(homePlainText(randomYoutube.title || "YouTube 강의"), 70)), homeExplicitNew(randomYoutube) ? /* @__PURE__ */ React.createElement(WebRStatusBadge, { tone: "new", label: "NEW" }) : null), /* @__PURE__ */ React.createElement("span", { class: "mt-1 block text-xs font-semibold text-slate-400" }, homeFormatDate(randomYoutube.created_at || randomYoutube.published_at))) : /* @__PURE__ */ React.createElement("a", { href: "/workshop/youtube/", class: "mx-4 mb-4 block rounded-md border border-dashed border-slate-200 px-3 py-3 text-sm font-semibold text-slate-500 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700" }, "YouTube 보기"));
  }
  function LivePanel() {
  ...
  function StatPanel() {
    const rows = [
      { label: "총 가입자 수", value: homeState.statistics.cnt_member, unit: "명", icon: "https://cdn.jsdelivr.net/gh/statground/Statground_CDN/assets3/images/svg/member.svg" },
      { label: "오늘의 방문자 수", value: homeState.statistics.cnt_visitor, unit: "명", icon: "https://cdn.jsdelivr.net/gh/statground/Statground_CDN/assets3/images/svg/visitors.svg" },
      { label: "오늘의 페이지 뷰", value: homeState.statistics.cnt_pageview, unit: "건", icon: "https://cdn.jsdelivr.net/gh/statground/Statground_CDN/assets3/images/svg/pageview.svg" }
    ];
    return /* @__PURE__ */ React.createElement("aside", { class: "rounded-lg border border-slate-100 bg-slate-50/80 p-4 shadow-sm" }, /* @__PURE__ */ React.createElement("h3", { class: "text-base font-extrabold text-slate-950" }, "Web-R 현황"), /* @__PURE__ */ React.createElement("div", { class: "mt-3 grid grid-cols-1 gap-2" }, rows.map((row) => /* @__PURE__ */ React.createElement("div", { key: row.label, class: "flex min-h-[72px] items-center gap-3 rounded-md border border-slate-100 bg-white px-3 py-2" }, /* @__PURE__ */ React.createElement("img", { src: row.icon, alt: "", class: "h-7 w-7 shrink-0", loading: "lazy" }), /* @__PURE__ */ React.createElement("span", { class: "min-w-0" }, /* @__PURE__ */ React.createElement("span", { class: "block text-xs font-bold text-slate-500" }, row.label), homeState.loading.statistics ? /* @__PURE__ */ React.createElement("span", { class: "mt-2 block h-5 w-24 rounded-full bg-slate-300 animate-pulse" }) : /* @__PURE__ */ React.createElement("span", { class: "mt-1 block text-xl font-extrabold text-slate-950" }, formattedStat(row.value, row.unit)))))));
  }
