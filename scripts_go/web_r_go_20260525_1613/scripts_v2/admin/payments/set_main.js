...
function Div_payer_table(props) {
  const baseRows = React.useMemo(() => paymentRowsFromData(props.data), [props.data]);
  const [search, setSearch] = React.useState("");
  const [remoteRows, setRemoteRows] = React.useState(null);
  ...
  const localRows = React.useMemo(() => {
    if (!query)
      return baseRows;
    return baseRows.filter((row) => paymentSearchHaystack(row).includes(query));
  }, [baseRows, query]);
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (!canceled) {
          setRemoteRows(paymentRowsFromData(result));
          setMessage("");
        }
      } catch (error) {
        if (!canceled) {
  ...
  const rows = remoteRows !== null ? remoteRows : localRows;
  const visibleRows = rows.slice(0, 100);
  return /* @__PURE__ */ React.createElement("div", { className: "w-full bg-white border border-gray-200 rounded-lg shadow" }, /* @__PURE__ */ React.createElement("div", { className: "p-4 bg-white rounded-lg md:p-8" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-3 md:flex-row md:items-center md:justify-between" }, /* @__PURE__ */ React.createElement("div", { className: "text-left" }, /* @__PURE__ */ React.createElement("p", { className: "text-2xl font-extrabold text-gray-900" }, "결제자 목록"), /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-sm text-gray-500" }, loading ? "검색 중" : rows.length.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "건")), /* @__PURE__ */ React.createElement("div", { className: "w-full md:w-80" }, /* @__PURE__ */ React.createElement("input", { type: "search", value: search, onChange: (event) => setSearch(event.target.value), placeholder: "이메일, 이름, 주문번호 검색", className: "h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" }))), message ? /* @__PURE__ */ React.createElement("p", { className: "mt-3 rounded-lg bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700" }, message) : null, /* @__PURE__ */ React.createElement("div", { className: "mt-4 overflow-x-auto rounded-lg border border-gray-200" }, /* @__PURE__ */ React.createElement("table", { className: "min-w-full divide-y divide-gray-200 text-left text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-50 text-xs font-bold uppercase tracking-wide text-gray-500" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3" }, "결제일"), /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3" }, "결제자"), /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3" }, "상품"), /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3 text-right" }, "결제액"), /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3" }, "상태"), /* @__PURE__ */ React.createElement("th", { className: "px-4 py-3" }, "주문번호"))), /* @__PURE__ */ React.createElement("tbody", { className: "divide-y divide-gray-100 bg-white" }, visibleRows.length > 0 ? visibleRows.map((row, index) => {
  ...
  }) : /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("td", { colSpan: 6, className: "px-4 py-10 text-center text-sm text-gray-500" }, "표시할 결제자가 없습니다.")))))), rows.length > visibleRows.length ? /* @__PURE__ */ React.createElement("p", { className: "mt-3 text-right text-xs text-gray-500" }, "상위 ", visibleRows.length, "건 표시") : null));
}
