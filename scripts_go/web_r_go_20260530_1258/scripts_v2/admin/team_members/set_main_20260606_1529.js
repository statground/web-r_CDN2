...
const TEAM_ADMIN_SCOPES = {
  ...
  tester_team: {
    title: "테스터 계정/팀",
    loading: "테스터 계정과 생성 팀을 불러오고 있습니다.",
...
function TeamSummaryMetric(props) {
  return React.createElement("div", { className: "rounded-md bg-slate-50 px-4 py-3 text-left" },
    React.createElement("dt", { className: "text-xl font-extrabold leading-tight text-slate-950" }, props.value),
    React.createElement("dd", { className: "mt-1 text-sm font-medium text-slate-500" }, props.title)
  );
}

function TeamSummaryGroup(props) {
  const columnsClass = props.columns === 1 ? "grid-cols-1" : (props.columns === 2 ? "grid-cols-2" : (props.columns === 4 ? "grid-cols-2 md:grid-cols-4" : "grid-cols-3"));
  return React.createElement("section", { className: "rounded-lg border border-slate-200 bg-white p-4 text-left" },
...
function SpecialAccountsApp(props) {
  ...
  const testerOwnedTeams = Array.isArray(data.tester_owned_teams) ? data.tester_owned_teams : [];
  ...
  function renderTesterTeamMember(member) {
    const isOwner = teamAdminNumber(member && member.is_owner) === 1;
    return React.createElement("tr", { key: teamAdminText(member && member.uuid) + (isOwner ? "-owner" : ""), className: "border-b last:border-0 hover:bg-slate-50" },
      ...
      React.createElement("td", { className: "px-3 py-2 font-semibold text-slate-950 whitespace-nowrap" }, teamAdminText(member && member.email) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminText(member && member.nickname) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminText(member && member.realname) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminText(member && member.role) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminDateOnly(member && member.expired_at) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminDateOnly(member && member.joined_at) || "-"),
      React.createElement("td", { className: "px-3 py-2 whitespace-nowrap" }, teamAdminStatus(member || {}))
    );
  }

  function renderTesterOwnedTeam(team) {
    const members = Array.isArray(team.members) ? team.members : [];
    return React.createElement("section", { key: teamAdminText(team.team_uuid), className: "rounded-lg border border-gray-200 bg-white shadow" },
      React.createElement("div", { className: "flex flex-col gap-4 p-4 md:p-6" },
        React.createElement("div", { className: "flex flex-col gap-2 md:flex-row md:items-start md:justify-between" },
          React.createElement("div", null,
            React.createElement("h3", { className: "text-xl font-extrabold text-slate-950" }, team.team_name || "이름 없는 테스트 팀"),
            React.createElement("p", { className: "text-sm text-slate-500" }, "팀장: ", team.owner_email || "-", " / ", team.owner_nickname || "-")
          ),
          React.createElement("div", { className: "flex flex-wrap gap-2 text-sm" },
            React.createElement("span", { className: "rounded-md border border-amber-200 bg-amber-50 px-3 py-1 text-amber-700" }, "테스터 생성 팀"),
        ...
        React.createElement("div", { className: "w-full overflow-x-auto" },
          React.createElement("table", { className: "min-w-full text-left text-xs text-gray-600 md:text-sm" },
            ...
            React.createElement("tbody", null,
              members.length ? members.map(renderTesterTeamMember) : React.createElement("tr", null,
                React.createElement("td", { colSpan: "8", className: "px-3 py-6 text-center text-gray-400" }, "등록된 팀원이 없습니다.")
              )
            )
          )
        )
  ...
  function renderTesterOwnedTeams() {
    if (specialScope !== "tester") return null;
    return React.createElement("section", { className: "flex flex-col gap-3" },
      React.createElement("div", { className: "flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4 shadow-sm" },
        React.createElement("h3", { className: "text-base font-extrabold text-slate-950" }, "테스터 생성 팀"),
        React.createElement("span", { className: "text-sm text-slate-500" }, teamAdminFormatNumber(testerOwnedTeams.length), "팀")
      ),
      testerOwnedTeams.length === 0
        ? React.createElement("div", { className: "rounded-lg border border-gray-200 bg-white p-8 text-center text-gray-500" }, "테스터 계정이 만든 팀이 없습니다.")
        : testerOwnedTeams.map(renderTesterOwnedTeam)
    );
  }
...
function TeamMembersApp(props) {
  ...
  function toggleTeam(teamUUID) {
    setOpenTeams(function (prev) {
      return Object.assign({}, prev, { [teamUUID]: !prev[teamUUID] });
    });
  }
  ...
  function renderMemberCard(team, member) {
    const isOwner = teamAdminNumber(member.is_owner) === 1;
    const isInternal = teamAdminIsInternal(team);
    const canRemove = isInternal && !isOwner;
    const removeKey = "remove:" + teamAdminText(team.team_uuid) + ":" + teamAdminText(member.uuid);
    const contextLabel = teamAdminMembershipContextLabel(member);
    const entitlementLines = teamAdminEntitlementLines(member);
    const status = teamAdminStatus(member);
    return React.createElement("article", {
      key: teamAdminText(member.uuid) + (isOwner ? "-owner-card" : "-card"),
      ...
      React.createElement("div", { className: "flex flex-col gap-2 md:flex-row md:items-start md:justify-between" },
        React.createElement("div", { className: "min-w-0" },
          React.createElement("div", { className: "mb-2 flex flex-wrap items-center gap-2" },
            React.createElement("span", { className: "inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold " + (isOwner ? "border-blue-200 bg-blue-50 text-blue-700" : "border-slate-200 bg-white text-slate-700") }, isOwner ? "팀장" : "팀원"),
            React.createElement("span", { className: "inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold " + (status === "활성" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-rose-200 bg-rose-50 text-rose-700") }, status)
          ),
          React.createElement("p", { className: "break-all text-sm font-extrabold text-slate-950" }, member.email || "-"),
          React.createElement("p", { className: "mt-1 break-words text-xs text-slate-500" }, member.nickname || "-", " / ", member.realname || "-")
        ),
        isInternal ? React.createElement("div", { className: "shrink-0" },
          canRemove ? React.createElement("button", {
            type: "button",
            disabled: loadingAction === removeKey,
            onClick: function () { removeInternalMember(team, member); },
            className: "rounded-md border border-rose-200 bg-white px-2.5 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50"
          }, loadingAction === removeKey ? "처리 중" : "해제") : null
        ) : null
      ),
      React.createElement("dl", { className: "mt-3 grid grid-cols-1 gap-2 text-xs md:grid-cols-4" },
        React.createElement("div", { className: "min-w-0 rounded-md bg-slate-50 px-3 py-2" },
          React.createElement("dt", { className: "font-semibold text-slate-500" }, "등급"),
          React.createElement("dd", { className: "mt-1 break-words font-bold text-slate-900" }, member.role || "-")
        ),
        React.createElement("div", { className: "min-w-0 rounded-md bg-slate-50 px-3 py-2" },
          React.createElement("dt", { className: "font-semibold text-slate-500" }, "만료일"),
          React.createElement("dd", { className: "mt-1 break-words font-bold text-slate-900" }, teamAdminDateOnly(member.expired_at) || "-")
        ),
        React.createElement("div", { className: "min-w-0 rounded-md bg-slate-50 px-3 py-2" },
          React.createElement("dt", { className: "font-semibold text-slate-500" }, "등록일"),
          React.createElement("dd", { className: "mt-1 break-words font-bold text-slate-900" }, teamAdminDateOnly(member.joined_at) || "-")
        ),
        React.createElement("div", { className: "min-w-0 rounded-md bg-slate-50 px-3 py-2" },
          React.createElement("dt", { className: "font-semibold text-slate-500" }, "권한"),
          React.createElement("dd", { className: "mt-1 flex flex-col gap-1" },
            contextLabel ? React.createElement("span", { className: "inline-flex w-fit rounded-full border px-2 py-0.5 text-[11px] font-semibold " + teamAdminMembershipContextClass(member) }, contextLabel) : React.createElement("span", { className: "font-bold text-slate-900" }, "-"),
            entitlementLines.map(function (line) {
              return React.createElement("span", { key: line, className: "break-words text-[11px] font-medium text-slate-500" }, line);
            })
          )
        )
      )
  ...
  function renderTeam(team, index) {
    ...
    const teamUUID = teamAdminText(team.team_uuid) || teamAdminText(team.team_name) || ("team-" + index);
    ...
    const isOpen = openTeams[teamUUID] === true;
    return React.createElement("section", { key: teamUUID, className: "min-w-0 w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm" },
      React.createElement("button", {
        type: "button",
        onClick: function () { toggleTeam(teamUUID); },
        "aria-expanded": isOpen ? "true" : "false",
        className: "flex w-full flex-col gap-3 p-4 text-left transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 md:p-5"
      },
        React.createElement("div", { className: "flex flex-col gap-2 md:flex-row md:items-start md:justify-between" },
          React.createElement("div", { className: "min-w-0" },
            React.createElement("h3", { className: "break-words text-lg font-extrabold text-slate-950 md:text-xl" }, team.team_name || "이름 없는 팀"),
            React.createElement("p", { className: "mt-1 text-sm text-slate-500" }, "팀장: ", ownerLabel)
          ),
          React.createElement("div", { className: "flex flex-wrap gap-2 text-sm" },
            React.createElement("span", { className: "rounded-md border border-slate-200 bg-slate-50 px-3 py-1" }, "좌석 ", teamAdminFormatNumber(team.seat_used), " / ", teamAdminFormatNumber(team.seat_limit)),
            React.createElement("span", { className: "rounded-md border border-indigo-200 bg-indigo-50 px-3 py-1 text-indigo-700" }, "팀원 등급 ", team.member_role_name || "정회원"),
            ...
            React.createElement("span", { className: "rounded-md border border-slate-200 bg-slate-50 px-3 py-1" }, "팀원 ", teamAdminFormatNumber(team.active_member_count), "명"),
            React.createElement("span", { className: "inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-3 py-1 font-semibold text-slate-700" },
              React.createElement("span", { "aria-hidden": "true", className: "inline-flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-xs text-slate-700" }, isOpen ? "⌃" : "⌄"),
              isOpen ? "접기" : "펼치기"
            )
          )
        ),
        React.createElement("div", { className: "flex flex-wrap gap-2 text-xs text-slate-500" },
          React.createElement("span", null, "만료 ", teamAdminDateOnly(team.expires_at) || "-"),
          React.createElement("span", null, "최근 결제 ", teamAdminDateOnly(team.last_payment_at) || "-"),
          pausedPersonalCount > 0 ? React.createElement("span", { className: "font-semibold text-violet-700" }, "개인권 보존 ", teamAdminFormatNumber(pausedPersonalCount), "명") : null
        )
      ),
      isOpen ? React.createElement("div", { className: "flex flex-col gap-4 border-t border-slate-200 bg-white p-4 md:p-5" },
        ...
        React.createElement("section", { className: "min-w-0 rounded-md border border-slate-200 bg-slate-50 p-3" },
          React.createElement("h4", { className: "mb-3 text-sm font-extrabold text-slate-900" }, "팀원 목록"),
          members.length === 0
            ? React.createElement("div", { className: "rounded-md bg-white px-3 py-6 text-center text-gray-400" }, "등록된 팀원이 없습니다.")
            : React.createElement("div", { className: "grid min-w-0 grid-cols-1 gap-3 xl:grid-cols-2" }, members.map(function (member) { return renderMemberCard(team, member); }))
        )
      ) : null
    );
  }
  ...
  function renderSummaryMetrics() {
    ...
    return React.createElement("div", { className: "w-full bg-white border border-gray-200 rounded-lg shadow" },
      React.createElement("div", { className: "p-4 bg-white rounded-lg md:p-8 text-center" },
        React.createElement("h5", { className: "mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900" },
          React.createElement("span", { className: "text-blue-600" }, scope.title)
        ),
        React.createElement("div", { className: "grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3" },
      React.createElement(TeamSummaryGroup, { title: "팀 구성", columns: 4 },
  ...
  return React.createElement("div", { className: "grid grid-cols-1 items-start w-full px-3 py-5 md:grid-cols-12 md:px-6 xl:px-8 2xl:px-10" },
    React.createElement(Div_operation_menu, null),
    React.createElement("div", { className: "min-w-0 w-full max-w-none md:col-span-10" },
      renderSummaryMetrics(),
      React.createElement("div", { className: "min-w-0 w-full bg-white border border-gray-200 rounded-lg shadow" },
        React.createElement("div", { className: "p-4 bg-white rounded-lg md:p-6" },
          React.createElement("h5", { className: "mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900" }, "팀 검색"),
          actionError ? React.createElement("p", { className: "rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700" }, actionError) : null,
          React.createElement("input", {
            type: "search",
            value: search,
      ...
      React.createElement("div", { className: "min-w-0 w-full bg-white border border-gray-200 rounded-lg shadow" },
        React.createElement("div", { className: "p-4 bg-white rounded-lg md:p-6" },
          React.createElement("h5", { className: "mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900" }, "팀 목록"),
          React.createElement("div", { className: "flex w-full flex-col gap-3" },
...
async function set_main() {
  ...
  try {
    ...
    if (scope.specialScope) {
      ...
      if (scope.specialScope === "tester") {
        const teamData = await fetch("/admin/ajax_get_admin_team_members/", { method: "POST", credentials: "same-origin" }).then(function (res) { return res.json(); });
        const teams = Array.isArray(teamData && teamData.teams) ? teamData.teams : [];
        data.tester_owned_teams = teams.filter(function (team) { return teamAdminText(team.team_kind) === "tester_owned"; });
      }
    } else {
