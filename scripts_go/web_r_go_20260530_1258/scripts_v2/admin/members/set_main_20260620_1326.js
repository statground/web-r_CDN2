(function () {
  const SUBTAB_STYLE_ID = "webr-admin-members-bootstrap-subtabs-20260620-1326";
  ...
  function isBaseMembersMain(fn) {
    const text = functionText(fn);
    return text.indexOf("renderAdminMembersPayload") >= 0 ||
      text.indexOf("defaultAdminMembersPayload") >= 0 ||
      text.indexOf("/admin/ajax_get_admin_members_roles/") >= 0;
  }
  ...
  function bareGetMain() {
    try {
      if (typeof get_main === "function") return get_main;
    } catch (error) {
      return null;
    }
    return null;
  ...
  function neutralizeBasePayloadRenderer() {
    try {
      ...
      // The modern split renderer does not depend on the old payload renderer.
    }
  }

  function stabilizeMembersMain(fn) {
    window.get_main = fn;
    try {
      get_main = fn;
    } catch (error) {
  ...
  function membersMainCandidate() {
    ...
    const candidates = [
      bareGetMain(),
      typeof window.__webrAdminMembersPreferredMain === "function" ? window.__webrAdminMembersPreferredMain : null,
      typeof window.get_main === "function" ? window.get_main : null
    ];
    for (let i = 0; i < candidates.length; i += 1) {
      const fn = candidates[i];
      if (typeof fn !== "function") continue;
      if (isBaseMembersMain(fn)) continue;
      return stabilizeMembersMain(fn);
    }
    return null;
  }
  ...
  function waitForMembersMain(attempt) {
    ...
    if (attempt >= 80) {
      return Promise.reject(new Error("admin members split get_main is not registered"));
    }
