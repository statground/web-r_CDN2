...
function AdminMembersManageMain(props) {
    ...
    return membersRaw.filter((member) => {
      if (q) {
        const haystack = [
          ...
          member.auth_provider_label,
          member.google_email,
          member.google_name,
          member.google_subject,
          adminMemberContextLabel(member),
          ...
          member.gender,
          adminMemberStatusText(member),
          ...adminMemberPaymentHistory(member).flatMap((payment) => [
            payment && payment.order_id,
            payment && payment.product_name,
      ...
      if (statusFilters.length > 0 && !statusFilters.includes(memberStatus))
        return false;
      if (paymentFilter === "has_payment" && adminMemberPaymentCount(member) <= 0)
        return false;
      return true;
    });
  }, [membersRaw, search, roleFilters, statusFilters, contextFilter, paymentFilter]);
  ...
  }, [loadMemberSections, props.skipInitialLoad]);
  React.useEffect(() => setPage(1), [search, roleFilters, statusFilters, contextFilter, paymentFilter, sortState]);
  React.useEffect(() => {
