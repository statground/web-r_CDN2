...
function adminMembersFetchSection(section) {
  const endpoint = ADMIN_MEMBER_SECTION_ENDPOINTS[section];
  if (!endpoint)
    return Promise.resolve({});
  return fetch(endpoint, { method: "POST", credentials: "same-origin" }).then((res) => res.json());
}
function AdminMembersManageMain(props) {
  ...
  const pageSize = 20;
  const loadMemberSections = React.useCallback((sections) => {
    ...
    return Promise.all(selectedSections.map((section) => adminMembersFetchSection(section).then((payload) => {
  ...
  function reloadMembers() {
    return loadMemberSections(ADMIN_MEMBER_SECTION_NAMES);
  }
