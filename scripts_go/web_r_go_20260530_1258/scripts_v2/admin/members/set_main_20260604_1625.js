...
function draw_chart(inputData, activeTabId) {
  ...
  const { categories, legend, series, stacked } = chartData;
  ...
  const option = {
    ...
    yAxis: [{ type: "value", name: stacked ? "\uD68C\uC6D0 \uC218" : "\uAC00\uC785\uC790 \uC218" }],
...
function normalizeMembers(obj) {
  const rows = Object.values(obj || {}).map((row) => {
    ...
    return { date, role, cnt: Number(cnt) || 0 };
  }).filter((row) => row.date).sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
  if (rows.length === 0)
    return { categories: [], legend: [], series: [], stacked: false };
