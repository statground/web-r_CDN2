(function () {
  const STYLE_ID = "webr-admin-members-graph-fix-style-20260606-1234";
  ...
  function drawRoleGraph(period, force) {
    ...
    whenEChartsReady(function () {
      ...
      if (roles.indexOf("준회원") >= 0) legendSelected["준회원"] = false;
      const option = {
        title: { text: "등급별 회원 수 추이", left: "center", top: 0, textStyle: { fontSize: 24, fontWeight: "700" } },
        legend: { data: roles.concat(hasPaymentLine ? ["결제 건수"] : []), top: 38, selected: legendSelected },
