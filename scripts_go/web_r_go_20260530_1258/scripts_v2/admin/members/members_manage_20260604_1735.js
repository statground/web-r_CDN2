...
function AdminMembersManageMain(props) {
  ...
  const paymentCounts = React.useMemo(() => {
    return {
      all: memberFilterCount("payment", "all", membersRaw.length),
  ...
  function goToMemberPage(page) {
    if (listLoading)
      return;
    setPendingPage(page);
    loadMemberSections(["list"], { page, includeCounts: false }).finally(() => setPendingPage(0));
  }
  function renderPageButton(label, page, disabled) {
    const spinning = listLoading && pendingPage === page;
    return /* @__PURE__ */ React.createElement("button", { type: "button", className: "inline-flex min-h-[32px] items-center justify-center gap-2 rounded border px-3 py-1 disabled:opacity-50", onClick: () => goToMemberPage(page), disabled: disabled || listLoading }, spinning ? /* @__PURE__ */ React.createElement("span", { className: "h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-blue-600", "aria-hidden": "true" }) : null, /* @__PURE__ */ React.createElement("span", null, label));
  }
