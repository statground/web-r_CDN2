(function () {
  ...
  function adminMembersPayloadFailed(payload) {
    return !payload || payload.ok === false || payload.pending === true || payload.fallback === true || payload.role_graph_pending === true;
  }
  ...
  function fetchMemberSection(section, body) {
    if (typeof adminMembersFetchSection === "function") {
      return adminMembersFetchSection(section, body).then(function (payload) {
        if (adminMembersPayloadFailed(payload)) {
          throw new Error(payload && payload.error ? payload.error : "admin members payload failed");
        }
        return payload;
      });
    }
    ...
    return fetch(endpoint, options).then(function (res) { return res.json(); }).then(function (payload) {
      if (adminMembersPayloadFailed(payload)) {
        throw new Error(payload && payload.error ? payload.error : "admin members payload failed");
      }
      return payload;
