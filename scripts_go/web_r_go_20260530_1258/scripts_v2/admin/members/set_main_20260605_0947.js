...
function Div_main_skeleton(props) {
  return /* @__PURE__ */ React.createElement("div", { class: "webr-admin-members-dashboard grid grid-cols-1 md:grid-cols-12 justify-center item-center w-full px-2 py-4 md:px-6" }, /* @__PURE__ */ React.createElement(Div_operation_menu, null), /* @__PURE__ */ React.createElement("div", { class: "md:col-span-10 justify-center item-center" }, /* @__PURE__ */ React.createElement(
    "div",
    {
...
const MEMBER_ROLE_CHART_ORDER = ["\uC900\uD68C\uC6D0", "\uC815\uD68C\uC6D0", "VIP\uD68C\uC6D0", "\uD300/\uC815\uD68C\uC6D0", "\uD300/VIP\uD68C\uC6D0", "\uAE30\uAD00/\uD300 \uD68C\uC6D0"];
const MEMBER_ROLE_CHART_COLORS = {
  "\uC900\uD68C\uC6D0": "#94a3b8",
  "\uC815\uD68C\uC6D0": "#2563eb",
  "VIP\uD68C\uC6D0": "#7c3aed",
  "\uD300/\uC815\uD68C\uC6D0": "#0f766e",
...
function draw_chart(inputData, activeTabId) {
  ...
  const chartData = normalizeMembers(inputData);
  const { categories, legend, series, stacked, hasPaymentLine } = chartData;
  const zoomStart = chartZoomStart(categories, activeTabId);
  const option = {
    title: {
      text: stacked ? "\uB4F1\uAE09\uBCC4 \uD68C\uC6D0 \uC218 \uCD94\uC774" : "\uAC00\uC785\uC790 \uC218 \uCD94\uC774 \uADF8\uB798\uD504",
      left: "center",
      top: 0,
      textStyle: { fontSize: 24, fontWeight: "700" }
    },
    legend: { data: legend, top: 36, selected: stacked ? { "\uC900\uD68C\uC6D0": false } : {} },
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" } },
    toolbox: { right: 10, feature: { dataZoom: { yAxisIndex: "none" }, restore: {}, saveAsImage: {} } },
    grid: { left: 60, right: hasPaymentLine ? 80 : 60, top: 80, bottom: 60 },
    xAxis: { type: "category", data: categories, axisLabel: { interval: "auto", hideOverlap: true } },
    yAxis: hasPaymentLine ? [
      { type: "value", name: stacked ? "\uD68C\uC6D0 \uC218" : "\uAC00\uC785\uC790 \uC218" },
...
function normalizeMembers(obj) {
  const rows = Object.values(obj || {}).map((row) => {
    ...
    const role = String(row && (row.role ?? row.role_name ?? row.name ?? row.ROLE) || "\uAC00\uC785\uC790 \uC218").trim() || "\uAC00\uC785\uC790 \uC218";
    const paymentCnt = row && (row.payment_cnt ?? row.payment_count ?? row.payments ?? row.PAYMENT_CNT);
    return { date, role, cnt: Number(cnt) || 0, paymentCnt: Number(paymentCnt) || 0 };
  }).filter((row) => row.date).sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
  if (rows.length === 0)
    return { categories: [], legend: [], series: [], stacked: false, hasPaymentLine: false };
  const stacked = rows.some((row) => row.role !== "\uAC00\uC785\uC790 \uC218");
  ...
  for (let d = new Date(start); d <= end; inc(d, granularity)) {
    categories.push(formatDate(d, granularity));
  }
  ...
  const paymentData = categories.map((key) => Number(paymentMap.get(key)) || 0);
  const hasPaymentLine = paymentData.some((value) => value > 0);
  if (!stacked) {
    const map = new Map(rows.map((r) => [r.date, r.cnt || 0]));
    const series = [{
      name: "\uAC00\uC785\uC790 \uC218",
      type: "bar",
      data: categories.map((key) => map.has(key) ? Number(map.get(key)) || 0 : 0),
      barMaxWidth: 28,
    ...
    return {
      categories,
      legend: hasPaymentLine ? ["\uAC00\uC785\uC790 \uC218", "\uACB0\uC81C \uAC74\uC218"] : ["\uAC00\uC785\uC790 \uC218"],
      stacked: false,
      hasPaymentLine,
      series
    };
  ...
  const roles = Array.from(roleSet).sort((a, b) => memberRoleChartOrder(a) - memberRoleChartOrder(b) || a.localeCompare(b));
  const map = new Map();
  rows.forEach((row) => {
    const key = `${row.date}\u0000${row.role}`;
    map.set(key, (Number(map.get(key)) || 0) + (row.cnt || 0));
  });
  const series = roles.map((role) => ({
    name: role,
    type: "bar",
    stack: "member_grade",
    data: categories.map((date) => Number(map.get(`${date}\u0000${role}`)) || 0),
    barMaxWidth: 32,
    itemStyle: { color: MEMBER_ROLE_CHART_COLORS[role] || "#64748b" }
  }));
  if (hasPaymentLine) {
    series.push({
      name: "\uACB0\uC81C \uAC74\uC218",
  ...
  return {
    categories,
    legend: hasPaymentLine ? roles.concat(["\uACB0\uC81C \uAC74\uC218"]) : roles,
    stacked: true,
    hasPaymentLine,
    series
  };
}
function memberRoleChartOrder(role) {
  const idx = MEMBER_ROLE_CHART_ORDER.indexOf(role);
  return idx >= 0 ? idx : 90;
}
function inc(d, g) {
  if (g === "year")
