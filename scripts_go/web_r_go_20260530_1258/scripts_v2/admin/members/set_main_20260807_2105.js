(function () {
  const STYLE_ID = "webr-admin-members-stable-bootstrap-20260807-2105";
  const MODE_ATTR = "data-webr-admin-members-mode";
  let postRenderScheduled = false;
  let listRetryRunning = false;
  ...
  function adminMembersFailureFlag(value) {
    if (value === true || value === 1) return true;
    const normalized = String(value == null ? "" : value).trim().toLowerCase();
    return normalized === "1" || normalized === "true" || normalized === "yes";
  }

  function adminMembersPayloadFailed(payload, section) {
    if (!payload || typeof payload !== "object" || Array.isArray(payload)) return true;
    if (payload.ok === false || String(payload.ok == null ? "" : payload.ok).trim().toLowerCase() === "false") return true;
    const failureFlags = ["pending", "fallback", "failed", "partial", "stale_fallback", "snapshot_stale", "role_graph_pending"];
    if (failureFlags.some(function (key) { return adminMembersFailureFlag(payload[key]); })) return true;
    const status = String(payload.status || payload.state || "").trim().toLowerCase();
    if (["failed", "failure", "error", "pending", "fallback", "unavailable"].indexOf(status) >= 0) return true;
    if (section === "list") {
      if (!Object.prototype.hasOwnProperty.call(payload, "list_members")) return true;
      if (!payload.list_members || typeof payload.list_members !== "object") return true;
    }
    return false;
  }

  function adminMembersUnavailableError(section) {
    const error = new Error("admin members " + (section || "section") + " unavailable");
    error.name = "AdminMembersUnavailableError";
    return error;
  }

  function installStableSectionFetchGuard() {
    let baseFetch = null;
    try {
      if (typeof adminMembersFetchSection === "function") baseFetch = adminMembersFetchSection;
    } catch (error) {
      baseFetch = null;
    }
    if (!baseFetch || baseFetch.__webrAdminMembersStableGuard === true) return;
    const guardedFetch = function (section, body) {
      return Promise.resolve().then(function () {
        return baseFetch(section, body);
      }).then(function (payload) {
        if (adminMembersPayloadFailed(payload, section)) throw adminMembersUnavailableError(section);
        return payload;
      });
    };
    guardedFetch.__webrAdminMembersStableGuard = true;
    window.adminMembersFetchSection = guardedFetch;
    try {
      adminMembersFetchSection = guardedFetch;
    } catch (error) {
      // The window binding is sufficient when the global binding is read-only.
    }
  }

  function fetchMemberSection(section, body) {
    if (typeof adminMembersFetchSection === "function") {
      return adminMembersFetchSection(section, body).then(function (payload) {
        if (adminMembersPayloadFailed(payload, section)) throw adminMembersUnavailableError(section);
        return payload;
      });
    }
    ...
    return fetch(endpoint, options).then(function (res) {
      if (!res.ok) throw adminMembersUnavailableError(section);
      return res.json();
    }).then(function (payload) {
      if (adminMembersPayloadFailed(payload, section)) throw adminMembersUnavailableError(section);
      return payload;
  ...
  function listFailureCards() {
    return [
      React.createElement("div", {
        key: "failed-list",
        className: "w-full rounded-lg border border-gray-200 bg-white shadow",
        "data-webr-members-card": "list"
      }, React.createElement("div", { className: "rounded-lg bg-white p-4 md:p-6" },
        React.createElement("div", {
          className: "mb-6 rounded-lg border border-rose-200 bg-rose-50 px-4 py-5 text-center",
          role: "alert"
        },
          React.createElement("p", { className: "text-base font-bold text-rose-800" }, "회원 목록을 불러오지 못했습니다"),
          React.createElement("p", { className: "mt-1 text-sm text-rose-700" }, "잠시 후 다시 시도해 주세요."),
          React.createElement("button", {
            type: "button",
            className: "mt-4 inline-flex min-h-[40px] items-center justify-center rounded-lg border border-rose-300 bg-white px-4 py-2 text-sm font-semibold text-rose-800 hover:bg-rose-100 disabled:cursor-wait disabled:opacity-60",
            disabled: listRetryRunning,
            onClick: retryStableMembersList
          }, listRetryRunning ? "다시 불러오는 중" : "다시 시도")
        ),
        React.createElement("div", { className: "pointer-events-none select-none opacity-55", "aria-hidden": "true" }, listSkeleton())
      ))
    ];
  }
  ...
  function renderMembersListFailure(mount) {
    if (!mount) return false;
    if (typeof ReactDOM !== "object" && typeof ReactDOM !== "function") return false;
    if (typeof React !== "object" && typeof React !== "function") return false;
    const Menu = stableOperationMenu();
  ...
  async function loadStableMembersList(mount) {
    window.__webrAdminMembersListTouched = false;
    window.__webrAdminMembersRenderKey = "list-fast";
    let listData = null;
    try {
      listData = await fetchMemberSection("list", initialMembersListBody(false));
    } catch (error) {
      console.warn("admin members stable list request failed");
      listRetryRunning = false;
      renderMembersListFailure(mount);
      return false;
    }
    ...
    fetchMemberSection("list", initialMembersListBody(true)).then(function (countsData) {
      ...
      console.warn("admin members stable list counts request failed");
    });
    return true;
  }

  async function retryStableMembersList() {
    if (listRetryRunning) return;
    const mount = document.getElementById("div_main");
    if (!mount) return;
    listRetryRunning = true;
    renderStableMembersSkeleton(mount);
    try {
      await loadStableMembersList(mount);
    } finally {
      listRetryRunning = false;
    }
  }
  ...
  async function runStableMembersMain() {
    ...
    stableOperationMenu();
    neutralizeBasePayloadRenderer();
    installStableSectionFetchGuard();
  ...
  window.__webrAdminMembersBootstrapGuard = {
    ...
    retryMembersList: retryStableMembersList
  };
