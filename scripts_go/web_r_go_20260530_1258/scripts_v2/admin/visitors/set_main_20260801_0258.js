...
function adminVisitorTrafficStatus(data) {
  const status = String((data || {}).traffic_status || "available").trim().toLowerCase();
  return status === "unavailable" || status === "stale" ? status : "available";
}

function adminVisitorFetchedAt(data) {
  const raw = String((data || {}).fetched_at || "").trim();
  if (!raw) return "";
...
function Div_sub_card(props) {
  ...
  const displayValue = props.unavailable ? "집계 일시 중단" : adminVisitorFormat(props.value) + (props.unit || "");
  return React.createElement(
    "div",
    { className: "flex flex-col justify-center items-center rounded-xl space-y-2 w-full p-6" },
    React.createElement("dt", { className: "mb-2 text-3xl md:text-3xl font-extrabold" }, displayValue),
    React.createElement(
      "dd",
...
function Div_counts_status(props) {
  const status = adminVisitorTrafficStatus(props.data);
  const fetchedAt = adminVisitorFetchedAt(props.data);
  const staleFallback = !!((props.data || {}).stale_fallback || (props.data || {}).snapshot_stale);
  const delayed = status !== "available" || staleFallback;
  return React.createElement(
    "div",
    { className: "mb-4 flex flex-col gap-3 rounded-lg border px-4 py-3 md:flex-row md:items-center md:justify-between " + (delayed ? "border-amber-200 bg-amber-50 text-amber-900" : "border-slate-200 bg-slate-50 text-slate-700") },
    React.createElement(
      "div",
      null,
      React.createElement("p", { className: "text-sm font-semibold" }, delayed ? "방문 집계가 지연되어 마지막 확인값을 유지하고 있습니다." : "첫 화면의 Web-R 현황과 같은 오늘 집계 기준입니다."),
      React.createElement("p", { className: "mt-1 text-xs" }, "총계·연간·월간 카드는 누적값이며, 첫 화면과 비교할 값은 ‘오늘’ 카드입니다.", fetchedAt ? " 조회 시각: " + fetchedAt : "")
    ),
    React.createElement(
      "button",
      {
        ...
        className: "shrink-0 rounded-lg border border-blue-600 bg-white px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-50 disabled:cursor-wait disabled:opacity-60"
      },
      props.refreshing ? "최신 집계 확인 중…" : "최신 집계 새로고침"
    )
  );
}
...
function Div_main(props) {
  ...
  const countsRefreshing = !!loading.counts;
  const countsLoading = countsRefreshing && !data.count;
  const graphLoading = !!loading.graph;
  const trafficUnavailable = adminVisitorTrafficStatus(data) !== "available";
  return React.createElement(
    "div",
    ...
    React.createElement(
      ...
      React.createElement(Div_counts_status, { data, refreshing: countsRefreshing, onRefresh: props.onRefreshCounts }),
      React.createElement(
        Div_metric_group,
        ...
        React.createElement(Div_sub_card, { title: "오늘 방문자 수", value: adminVisitorMetric(data, "val_visitor_daily"), unit: "명", unavailable: trafficUnavailable })
      ),
      React.createElement(
        Div_metric_group,
        ...
        React.createElement(Div_sub_card, { title: "오늘 페이지 뷰", value: adminVisitorMetric(data, "val_pageview_daily"), unit: "건", unavailable: trafficUnavailable })
      ),
      React.createElement(
        "div",
        { className: "w-full bg-white border border-gray-200 rounded-lg shadow" },
...
async function get_main() {
  ...
  const render = (drawGraph) => {
    ReactDOM.render(
      React.createElement(Div_main, { data, loading, errors, onRefreshCounts: loadCounts }),
      mount,
      () => {
  ...
  const loadSection = (key, url) => fetch(url, {
    method: "POST",
  ...
  function loadCounts() {
    ...
    loading = { ...loading, counts: true };
    errors = { ...errors, counts: false };
    render(false);
    countsRequest = loadSection("counts", "/admin/ajax_get_admin_visitors_counts/?fresh=" + Date.now()).finally(() => {
      countsRequest = null;
    });
