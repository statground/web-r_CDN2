...
function Div_main(props) {
  ...
  React.useEffect(() => {
    ...
    scheduleActiveUsersChart(rows, tabIdMap[activeTab], { excludeVisitorFlow, metricRows });
...
function buildActiveUsersMetrics(inputData, activeTabId, chartOptions) {
  chartOptions = chartOptions || {};
  const rows = normalizeActiveUsersRows(chartOptions.metricRows || inputData);
  const n = function(x) {
    return +x || 0;
  };
...
function activeUsersStepItems(m) {
  return [
    ...
    { name: "\uBC29\uBB38\u2192\uB85C\uADF8\uC778", value: m.cr2, numeratorLabel: "\uB85C\uADF8\uC778", numerator: m.login, numeratorUnit: "\uBA85", denominatorLabel: "\uBC29\uBB38\uC790", denominator: m.visit, denominatorUnit: "\uBA85" },
...
function renderActiveUsersFallbackCharts(inputData, activeTabId, chartOptions) {
  const m = buildActiveUsersMetrics(inputData, activeTabId, chartOptions);
...
function draw_chart(inputData, activeTabId, drawToken, chartOptions) {
  ...
  const last = rows[rows.length - 1] || {};
  const metricRows = normalizeActiveUsersRows(chartOptions.metricRows || rows);
  const metricLast = metricRows[metricRows.length - 1] || last;
  const isMonthly = activeTabId === "graph_tab_monthly" || last.type === "monthly";
  let cumSignup;
  if (typeof metricLast.cum_signup !== "undefined") {
    cumSignup = n(metricLast.cum_signup);
  } else {
    cumSignup = metricRows.reduce(function(sum, r) {
      return sum + n(r.cnt_signup);
    }, 0);
  }
  let cumVisit = 0;
  if (isMonthly) {
    cumVisit = metricRows.reduce(function(sum, r) {
      return sum + n(r.cnt_visit);
    }, 0);
  }
  const visit = n(metricLast.cnt_visit);
  const signup = n(metricLast.cnt_signup);
  const login = n(metricLast.cnt_login);
  const board = n(metricLast.cnt_board);
  const app = n(metricLast.cnt_app);
  const appWebR = n(metricLast.cnt_app_webr);
  const appWebR2 = n(metricLast.cnt_app_webr2);
  const appNotebook = n(metricLast.cnt_app_notebook);
  const hasAppSplit = ["cnt_app_webr", "cnt_app_webr2", "cnt_app_notebook"].some(function(key) {
    return typeof metricLast[key] !== "undefined";
  });
  const payment = n(metricLast.cnt_payment);
  let cr1, cr2, cr3, cr4, cr5;
  cr1 = rate(signup, visit);
  cr2 = rate(login, visit);
  ...
  const metric = { rows: metricRows, n, rate, last: metricLast, isMonthly, cumSignup, cumVisit, visit, signup, login, board, app, appWebR, appWebR2, appNotebook, hasAppSplit, payment, cr1, cr2, cr3, cr4, cr5 };
