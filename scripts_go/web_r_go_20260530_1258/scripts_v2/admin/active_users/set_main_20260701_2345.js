...
function activeUsersFlowItems(m) {
  const items = [
    { name: "\uBC29\uBB38\uC790", value: m.visit, unit: "\uBA85", order: 10 },
    { name: "\uC571 \uC0AC\uC6A9", value: m.app, unit: "\uBA85", order: 20 },
    { name: "\uAC00\uC785\uC790", value: m.signup, unit: "\uBA85", order: 30 },
    { name: "\uB85C\uADF8\uC778", value: m.login, unit: "\uBA85", order: 40 },
    { name: "\uAC8C\uC2DC\uD310 \uC774\uC6A9", value: m.board, unit: "\uBA85", order: 50 },
    { name: "\uACB0\uC81C", value: m.payment, unit: "\uAC74", order: 60 }
  ];
...
function renderActiveUsersFallbackCharts(inputData, activeTabId) {
  ...
  const flowItems = activeUsersFlowItems(m);
...
function draw_chart(inputData, activeTabId, drawToken) {
  ...
  const flowItems = activeUsersFlowItems(metric);
    ...
    try {
      ...
      tooltip: {
        ...
        formatter: function(p) {
          if (p.dataType === "edge") {
            return p.data.source + "\u2192" + p.data.target + ": " + (+p.data.value || 0).toLocaleString() + (flowUnitByName[p.data.target] || "\uBA85");
          }
      ...
      series: [
        {
          ...
          links: flowItems.slice(1).map(function(item, idx) {
            return { source: flowItems[idx].name, target: item.name, value: item.value };
          })
        }
      ]
      });
