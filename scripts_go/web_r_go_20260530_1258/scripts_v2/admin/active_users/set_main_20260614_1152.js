...
function Div_main(props) {
  ...
  React.useEffect(() => {
    ...
    if (!rows.length)
      return;
    scheduleActiveUsersChart(rows, tabIdMap[activeTab]);
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-12 justify-center items-top w-full px-[10px] py-[20px] md:px-[100px]" }, /* @__PURE__ */ React.createElement(Div_operation_menu, null), /* @__PURE__ */ React.createElement("div", { className: "md:col-span-10 justify-center items-center" }, /* @__PURE__ */ React.createElement("div", { className: classCard }, /* @__PURE__ */ React.createElement("div", { className: classWrap }, /* @__PURE__ */ React.createElement("dl", { className: "flex flex-col justify-center items-start w-full p-4 mx-auto text-gray-900" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center justify-between w-full mb-4 gap-3" }, /* @__PURE__ */ React.createElement("ul", { className: "flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 flex-1" }, /* @__PURE__ */ React.createElement("li", { className: "me-2", onClick: () => handleTabClick("daily") }, /* @__PURE__ */ React.createElement(
    ...
    Kpi,
    {
	  ...
	  )), /* @__PURE__ */ React.createElement("div", { className: "w-full mt-6 space-y-6" }, /* @__PURE__ */ React.createElement("div", { id: "div_funnel_graph", className: "w-full h-[420px] min-h-[420px] p-2", style: { height: "420px", minHeight: "420px" } }), /* @__PURE__ */ React.createElement("div", { id: "div_conv_graph", className: "w-full h-[380px] min-h-[380px] p-2", style: { height: "380px", minHeight: "380px" } }), /* @__PURE__ */ React.createElement("div", { id: "div_trend_graph", className: "w-full h-[420px] min-h-[420px] p-2", style: { height: "420px", minHeight: "420px" } })))))));
}
const GRAPH_TAB_IDS = [
  "graph_tab_daily",
  "graph_tab_monthly",
...
function scheduleActiveUsersChart(inputData, activeTabId) {
  ...
  const tick = function(attempt) {
    window.requestAnimationFrame(function() {
      ...
      if (activeUsersChartsReady() && window.echarts) {
        draw_chart(inputData, activeTabId, token);
        return;
      }
      ...
      if (attempt < 120) {
        ...
        renderActiveUsersFallbackCharts(inputData, activeTabId);
      }
    });
  };
  installActiveUsersChartObservers(inputData, activeTabId);
  tick(0);
}
function installActiveUsersChartObservers(inputData, activeTabId) {
...
function buildActiveUsersMetrics(inputData, activeTabId) {
  ...
  const cr1 = isMonthly ? rate(cumSignup, cumVisit) : rate(signup, visit);
  const cr2 = isMonthly ? rate(login, cumSignup) : rate(login, signup);
  const cr3 = rate(board, login);
  const cr4 = rate(app, visit);
  const cr5 = rate(payment, login);
  return { rows, n, rate, last, isMonthly, cumSignup, cumVisit, visit, signup, login, board, app, payment, cr1, cr2, cr3, cr4, cr5 };
}
function renderActiveUsersFallbackCharts(inputData, activeTabId) {
  ...
  if (flow) {
    ...
    flow.innerHTML = '<div class="h-full w-full rounded-lg border border-slate-200 bg-slate-50 p-5 text-slate-900">' +
      ...
      '<div class="grid h-[320px] grid-cols-1 gap-4 md:grid-cols-3">' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-4 shadow-sm"><div class="text-sm text-slate-500">방문자</div><div class="text-2xl font-extrabold">' + fmt(m.visit) + '명</div></div>' +
      '<div class="grid grid-rows-2 gap-4">' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-4 shadow-sm"><div class="text-sm text-slate-500">방문→앱</div><div class="text-2xl font-extrabold">' + fmt(m.app) + '명</div><div class="text-xs text-slate-500">' + pct(m.cr4) + '</div></div>' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-4 shadow-sm"><div class="text-sm text-slate-500">방문→가입</div><div class="text-2xl font-extrabold">' + fmt(m.signup) + '명</div><div class="text-xs text-slate-500">' + pct(m.cr1) + '</div></div>' +
      '</div>' +
      '<div class="grid grid-rows-3 gap-3">' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-3 shadow-sm"><div class="text-sm text-slate-500">가입→로그인</div><div class="text-xl font-extrabold">' + fmt(m.login) + '명</div><div class="text-xs text-slate-500">' + pct(m.cr2) + '</div></div>' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-3 shadow-sm"><div class="text-sm text-slate-500">로그인→결제</div><div class="text-xl font-extrabold">' + fmt(m.payment) + '건</div><div class="text-xs text-slate-500">' + pct(m.cr5) + '</div></div>' +
      '<div class="flex flex-col items-center justify-center rounded-lg bg-white p-3 shadow-sm"><div class="text-sm text-slate-500">로그인→게시판</div><div class="text-xl font-extrabold">' + fmt(m.board) + '명</div><div class="text-xs text-slate-500">' + pct(m.cr3) + '</div></div>' +
      '</div>' +
      '</div>' +
      '</div>';
  }
  ...
  if (conv) {
    ...
    conv.innerHTML = '<div class="h-full w-full rounded-lg border border-slate-200 bg-white p-5"><div class="mb-5 text-center text-base font-bold">단계별 전환율(%)</div>' + items.map(function(item) {
      const value = isNaN(item[1]) ? 0 : item[1];
      const width = Math.max(2, value / maxValue * 100);
      return '<div class="mb-4"><div class="mb-1 flex items-center justify-between text-sm"><span>' + item[0] + '</span><strong>' + pct(value) + '</strong></div><div class="h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-blue-500" style="width:' + width.toFixed(1) + '%"></div></div></div>';
    }).join("") + '</div>';
  }
  const trend = document.getElementById("div_trend_graph");
  if (trend) {
    prepareActiveUsersChartNode(trend, "div_trend_graph");
    trend.innerHTML = '<div class="h-full w-full rounded-lg border border-slate-200 bg-white p-5"><div class="mb-5 text-center text-base font-bold">활동 추이</div><div class="grid grid-cols-2 gap-3 md:grid-cols-6">' +
      [["방문", m.visit, "명"], ["앱", m.app, "명"], ["가입", m.signup, "명"], ["로그인", m.login, "명"], ["결제", m.payment, "건"], ["게시판", m.board, "명"]].map(function(item) {
        return '<div class="rounded-lg bg-slate-50 p-3 text-center"><div class="text-xs text-slate-500">' + item[0] + '</div><div class="text-lg font-extrabold">' + fmt(item[1]) + item[2] + '</div></div>';
      }).join("") + '</div></div>';
  }
}
function draw_chart(inputData, activeTabId, drawToken) {
  ...
  if (!window.echarts) {
    if (window.WebRAdminDashboard && window.WebRAdminDashboard.ensureECharts) {
      window.WebRAdminDashboard.ensureECharts(function() {
        scheduleActiveUsersChart(inputData, activeTabId);
      });
    } else {
  ...
  if (!rows.length) {
    ["kpi_visit_val", "kpi_signup_val", "kpi_login_val", "kpi_board_val", "kpi_app_val", "kpi_payment_val"].forEach(function(id) {
      const el = document.getElementById(id);
      if (el)
        el.innerText = "0";
    });
    ...
    try {
      ...
      tooltip: {
        ...
        formatter: function(p) {
          if (p.dataType === "edge") {
            return p.data.source + "\u2192" + p.data.target + ": " + (+p.data.value || 0).toLocaleString() + (p.data.target === "\uACB0\uC81C" ? "\uAC74" : "\uBA85");
          }
          return p.name;
        }
      },
      series: [
        {
          ...
          links: [
            { source: "\uBC29\uBB38\uC790", target: "\uC571 \uC0AC\uC6A9", value: app },
            { source: "\uBC29\uBB38\uC790", target: "\uAC00\uC785\uC790", value: signup },
            { source: "\uAC00\uC785\uC790", target: "\uB85C\uADF8\uC778", value: login },
            { source: "\uB85C\uADF8\uC778", target: "\uACB0\uC81C", value: payment },
            { source: "\uB85C\uADF8\uC778", target: "\uAC8C\uC2DC\uD310 \uC774\uC6A9", value: board }
          ]
        }
      ]
      });
  ...
  if (chartConv) {
    const cats = ["\uBC29\uBB38\u2192\uC571", "\uBC29\uBB38\u2192\uAC00\uC785", "\uAC00\uC785\u2192\uB85C\uADF8\uC778", "\uB85C\uADF8\uC778\u2192\uACB0\uC81C", "\uB85C\uADF8\uC778\u2192\uAC8C\uC2DC\uD310"];
    const vals = [
      isNaN(cr4) ? 0 : cr4,
      isNaN(cr1) ? 0 : cr1,
      isNaN(cr2) ? 0 : cr2,
    ...
    try {
      ...
      series: [
        { name: "\uBC29\uBB38", type: "line", data: sVisit, smooth: true },
	        ...
	        { name: "\uACB0\uC81C", type: "line", data: sPayment, smooth: true },
