...
function renderActiveUsersEmptyState(message) {
  ...
  ["kpi_visit_val", "kpi_signup_val", "kpi_login_val", "kpi_board_val", "kpi_app_val", "kpi_payment_val", "kpi_cum_signup_val"].forEach(function(id) {
...
function activeUsersStepItems(m) {
  return [
    ...
    { name: "\uBC29\uBB38\u2192\uAC00\uC785", value: m.cr1, numeratorLabel: "\uAC00\uC785\uC790", numerator: m.signup, numeratorUnit: "\uBA85", denominatorLabel: "\uBC29\uBB38\uC790", denominator: m.isMonthly ? m.cumVisit : m.visit, denominatorUnit: "\uBA85" },
    { name: "\uAC00\uC785\u2192\uB85C\uADF8\uC778", value: m.cr2, numeratorLabel: "\uB85C\uADF8\uC778", numerator: m.login, numeratorUnit: "\uBA85", denominatorLabel: "\uAC00\uC785\uC790", denominator: m.isMonthly ? m.cumSignup : m.signup, denominatorUnit: "\uBA85" },
...
function renderActiveUsersFallbackCharts(inputData, activeTabId) {
  ...
  if (trend) {
    ...
    trend.innerHTML = '<div class="h-full w-full rounded-lg border border-slate-200 bg-white p-5"><div class="mb-5 text-center text-base font-bold">활동 추이(전환 순서 기준)</div><div class="grid grid-cols-2 gap-3 md:grid-cols-6">' +
...
function draw_chart(inputData, activeTabId, drawToken) {
  ...
  const metric = { rows, n, rate, last, isMonthly, cumSignup, cumVisit, visit, signup, login, board, app, payment, cr1, cr2, cr3, cr4, cr5 };
  ...
  if (chartTrend) {
    ...
    try {
      ...
      series: trendSeries.map(function(item) {
        return { name: item.name, type: "line", data: item.data, smooth: true };
      })
      });
    } catch (err) {
      console.error(err);
