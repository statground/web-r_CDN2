...
(function () {
  ...
  function adminBalanceGuardRetryAfterMS(response) {
  ...
  function adminBalanceGuardFetchJSON() {
    ...
    return fetch("/admin/ajax_get_admin_balance_account/", {
      ...
      var retryAfterMS = adminBalanceGuardRetryAfterMS(response);
      if (!response.ok) {
        var httpError = new Error("ADMIN_BALANCE_HTTP_FAILURE");
        httpError.retryAfterMS = retryAfterMS;
        throw httpError;
      }
      return response.json().then(function (payload) {
        return { payload: payload, retryAfterMS: retryAfterMS };
      }, function (error) {
  ...
  async function adminBalanceGuardLoad() {
    ...
    for (var attempt = 0; attempt < ADMIN_BALANCE_GUARD_MAX_ATTEMPTS; attempt += 1) {
      if (attempt > 0) {
        await adminBalanceGuardDelay(Math.max(ADMIN_BALANCE_GUARD_RETRY_DELAYS_MS[attempt - 1], retryAfterMS));
      }
      try {
        var response = await adminBalanceGuardFetchJSON();
        var payload = response.payload;
        if (!adminBalanceGuardPayloadValid(payload)) {
          var invalidPayloadError = new Error("ADMIN_BALANCE_INVALID_PAYLOAD");
