...
(function () {
  var ADMIN_PAYMENTS_GUARD_CONTRACT = Object.freeze({
    ...
    order: "PARALLEL_SECTIONS_RENDER_AS_READY",
    chart: "ECHARTS_6_0_0_LAZY_SINGLE_PROMISE",
  ...
  var ADMIN_PAYMENTS_ECHARTS_URL = "https://cdn.jsdelivr.net/npm/echarts@6.0.0/dist/echarts.min.js";
  var ADMIN_PAYMENTS_ECHARTS_SCRIPT_ID = "webr-admin-payments-echarts-6-0-0";
  var ADMIN_PAYMENTS_ECHARTS_TIMEOUT_MS = 15000;
  ...
  function adminPaymentsGuardEChartsReady() {
    return !!(window.echarts && typeof window.echarts.init === "function");
  }

  function adminPaymentsGuardFinishECharts(resolve) {
    var dashboard = window.WebRAdminDashboard;
    if (dashboard && typeof dashboard.ensureECharts === "function") {
      dashboard.ensureECharts(function (instance) {
        resolve(instance || window.echarts);
      });
      return;
    }
  ...
  function adminPaymentsGuardFindEChartsScript() {
    var byID = document.getElementById(ADMIN_PAYMENTS_ECHARTS_SCRIPT_ID);
    if (byID) return byID;
    var scripts = document.getElementsByTagName("script");
    for (var index = 0; index < scripts.length; index += 1) {
      if (String(scripts[index].src || "") === ADMIN_PAYMENTS_ECHARTS_URL) return scripts[index];
    }
    return null;
  }

  function adminPaymentsGuardEnsureECharts() {
    if (adminPaymentsGuardEChartsReady()) {
      return Promise.resolve(window.echarts);
    }
    if (window.__webrAdminPaymentsEChartsPromise) {
      return window.__webrAdminPaymentsEChartsPromise;
    }

    window.__webrAdminPaymentsEChartsPromise = new Promise(function (resolve, reject) {
      var settled = false;
      var script = adminPaymentsGuardFindEChartsScript();
      var created = false;
      var timer = null;
      ...
      function succeed() {
        if (settled || !adminPaymentsGuardEChartsReady()) return false;
        settled = true;
        cleanup();
        adminPaymentsGuardFinishECharts(resolve);
        return true;
      }
      ...
      function fail(error) {
        ...
        settled = true;
        cleanup();
        if (created && script && script.parentNode) script.parentNode.removeChild(script);
        reject(error || new Error("ADMIN_PAYMENTS_ECHARTS_UNAVAILABLE"));
      }

      function onLoad() {
        if (!succeed()) fail(new Error("ADMIN_PAYMENTS_ECHARTS_INVALID_RUNTIME"));
      }

      function onError() {
        fail(new Error("ADMIN_PAYMENTS_ECHARTS_LOAD_FAILURE"));
      }

      if (!script) {
        script = document.createElement("script");
        script.id = ADMIN_PAYMENTS_ECHARTS_SCRIPT_ID;
        script.src = ADMIN_PAYMENTS_ECHARTS_URL;
        script.async = true;
      ...
      timer = window.setTimeout(function () {
        if (!succeed()) fail(new Error("ADMIN_PAYMENTS_ECHARTS_TIMEOUT"));
      }, ADMIN_PAYMENTS_ECHARTS_TIMEOUT_MS);
      if (created) (document.head || document.documentElement).appendChild(script);
      succeed();
    }).catch(function (error) {
      window.__webrAdminPaymentsEChartsPromise = null;
      throw error;
    });
    return window.__webrAdminPaymentsEChartsPromise;
  }
  ...
  function adminPaymentsGuardPayloadUnavailable(payload) {
    ...
    var failed = ["pending", "fallback", "partial", "stale_fallback", "snapshot_stale"].some(function (key) {
  ...
  function adminPaymentsGuardRetryAfterMS(response) {
    if (!response || !response.headers || typeof response.headers.get !== "function") return 0;
    var raw = String(response.headers.get("Retry-After") || "").trim();
    if (!raw) return 0;
    var seconds = Number(raw);
    var delayMS = Number.isFinite(seconds)
      ? seconds * 1000
      : Date.parse(raw) - Date.now();
    if (!Number.isFinite(delayMS) || delayMS <= 0) return 0;
    return Math.min(30000, Math.ceil(delayMS) + 250);
  }
  ...
  function adminPaymentsGuardFetchJSON(url) {
    ...
    return fetch(url, {
      ...
      var retryAfterMS = adminPaymentsGuardRetryAfterMS(response);
      if (!response.ok) {
        var httpError = new Error("ADMIN_PAYMENTS_HTTP_FAILURE");
  ...
  async function adminPaymentsGuardLoadSection(section, attemptCount, attemptOffset) {
    ...
    for (var localAttempt = 0; localAttempt < totalAttempts; localAttempt += 1) {
      ...
      if (globalAttempt > 0) {
        ...
        await adminPaymentsGuardDelay(Math.max(ADMIN_PAYMENTS_GUARD_RETRY_DELAYS_MS[delayIndex], retryAfterMS));
      }
      try {
        var response = await adminPaymentsGuardFetchJSON(ADMIN_PAYMENTS_GUARD_ENDPOINTS[section]);
        var payload = response.payload;
        if (!adminPaymentsGuardPayloadValid(section, payload)) {
          var invalidPayloadError = new Error("ADMIN_PAYMENTS_INVALID_" + section.toUpperCase() + "_PAYLOAD");
          invalidPayloadError.retryAfterMS = response.retryAfterMS;
          throw invalidPayloadError;
        }
        return payload;
      } catch (error) {
        lastError = error;
        retryAfterMS = Number.isFinite(Number(error && error.retryAfterMS))
  ...
  async function adminPaymentsGuardTrySection(section, attemptCount, attemptOffset, deferFailure) {
    try {
      ...
      if (section === "graph") await adminPaymentsGuardEnsureECharts();
  ...
  async function adminPaymentsGuardGetMain() {
    ...
    adminPaymentsGuardState.loadPromise = (async function () {
      ...
      await Promise.all([
        adminPaymentsGuardTrySection("amounts", 3, 0, false),
  ...
  window.WebRAdminPaymentsReadGuard = Object.freeze({
    ...
    ensureECharts: adminPaymentsGuardEnsureECharts,
