...
function Div_main(props) {
  ...
  const pageSize = 20;
  const countJoined = data.count_joined || {};
  const joinedValue = (key) => countJoined[key] && countJoined[key]["0"] != null ? countJoined[key]["0"] : 0;
  const roleRow = (idx, fallbackName) => data.count_role && data.count_role[String(idx)] ? data.count_role[String(idx)] : { name: fallbackName, cnt: 0 };
  React.useEffect(() => {
    if (!data.list_monthly)
      return;
  ...
  const filteredMembers = React.useMemo(() => {
    ...
    return membersRaw.filter((m) => {
      if (q) {
        ...
        if (!email.includes(q) && !nick.includes(q) && !real.includes(q)) {
          return false;
        }
      }
      ...
      if (blockedFilter === "blocked" && blockedVal !== 1)
        return false;
      if (blockedFilter === "active" && blockedVal === 1)
        return false;
      return true;
    });
...
function adminPostJSON(url) {
  return fetch(url, { method: "POST", credentials: "same-origin" }).then((res) => res.json());
}
...
async function get_main() {
  ReactDOM.render(
    /* @__PURE__ */ React.createElement(MembersDashboard, null),
