...
function TeamAdminCenteredSpinner(props) {
  return React.createElement("div", { className: "min-h-[60vh] w-full px-6 py-8 flex items-center justify-center" },
    React.createElement("div", { className: "flex flex-col justify-center items-center w-full space-y-4 text-center text-gray-500" },
      React.createElement("svg", { "aria-hidden": "true", className: "w-8 h-8 text-gray-200 animate-spin fill-blue-600", viewBox: "0 0 100 101", fill: "none", xmlns: "http://www.w3.org/2000/svg" },
      ...
      React.createElement("p", null, props.text || "관리자 여부를 확인하고 있습니다.")
    )
  );
}
...
function TeamMembersApp(props) {
  ...
  const payloadSummary = data.summary && typeof data.summary === "object" ? data.summary : null;
  const displaySummary = payloadSummary && Object.keys(payloadSummary).length ? Object.assign({}, scopedSummary, payloadSummary) : scopedSummary;
  ...
  return React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-12 justify-center item-center w-full px-[10px] py-[20px] md:px-[100px]" },
    ...
    React.createElement("div", { className: "md:col-span-10 justify-center item-center" },
      React.createElement("div", { className: "flex w-full flex-col gap-4" },
        ...
        data.loading_teams
          ? React.createElement("div", { className: "rounded-lg border border-gray-200 bg-white p-8 text-center text-gray-500" }, scope.loading)
...
function TeamMembersDashboard() {
  const [data, setData] = React.useState({ teams: [], summary: {}, loading_teams: true });
  const [error, setError] = React.useState("");
  React.useEffect(function () {
    teamAdminPost("/admin/ajax_get_admin_team_members_summary/", new URLSearchParams()).then(function (nextData) {
      setData(function (prev) {
        return Object.assign({}, prev, { summary: nextData.summary || {} });
      });
    }).catch(function (err) {
      console.error(err);
      setError("기관/팀 요약을 불러오지 못했습니다.");
    });
    fetch("/admin/ajax_get_admin_team_members/", { method: "POST", credentials: "same-origin" }).then(function (res) { return res.json(); }).then(function (nextData) {
      if (nextData && nextData.ok === false) {
        throw new Error("기관/팀 현황을 불러올 수 없습니다.");
      }
      setData(function (prev) {
        return Object.assign({}, prev, nextData || {}, { loading_teams: false });
      });
    }).catch(function (err) {
      console.error(err);
      setData(function (prev) {
        return Object.assign({}, prev, { loading_teams: false });
      });
      setError(err.message || "기관/팀 현황을 불러오지 못했습니다.");
    });
  }, []);
  return React.createElement(React.Fragment, null,
    error ? React.createElement("div", { className: "mx-auto mt-4 max-w-screen-lg rounded-md border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700" }, error) : null,
    React.createElement(TeamMembersApp, { data: data })
  );
}
...
async function set_main() {
  ...
  ReactDOM.render(React.createElement(TeamAdminCenteredSpinner, { text: "관리자 여부를 확인하고 있습니다." }), mount);
  try {
    ...
    if (scope.specialScope) {
      ...
      ReactDOM.render(React.createElement(TeamAdminCenteredSpinner, { text: scope.loading }), mount);
      const data = await teamAdminPost("/admin/ajax_get_special_accounts/", body);
      ReactDOM.render(React.createElement(SpecialAccountsApp, { data: data }), mount);
    } else {
      ReactDOM.render(React.createElement(TeamMembersDashboard, null), mount);
    }
  } catch (error) {
    console.error(error);
