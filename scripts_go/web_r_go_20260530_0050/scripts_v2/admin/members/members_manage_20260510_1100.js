...
function AdminMembersManageMain(props) {
  ...
  const contextCounts = React.useMemo(() => {
    const counts = { all: membersRaw.length, personal: 0, team: 0 };
  ...
  const contextFilterOptions = React.useMemo(() => [
    ...
    { value: "personal", label: "\uAC1C\uC778 \uAD8C\uD55C \uC0AC\uC6A9\uC911", count: contextCounts.personal },
    { value: "team", label: "\uD300 \uAD8C\uD55C \uC0AC\uC6A9\uC911", count: contextCounts.team }
  ], [contextCounts]);
    ...
    return membersRaw.filter((member) => {
      if (q) {
        const haystack = [
          ...
          member.active_personal_role_name,
          member.active_personal_expired_at,
          member.paused_personal_days,
          member.gender,
      ...
      if (roleFilters.length > 0 && !roleFilters.includes(memberRole))
        return false;
      if (contextFilter !== "all" && adminMemberContextType(member) !== contextFilter)
        return false;
  ...
  }, [membersRaw, search, roleFilters, statusFilters, contextFilter]);
  ...
  React.useEffect(() => setPage(1), [search, roleFilters, statusFilters, contextFilter, sortState]);
  React.useEffect(() => {
