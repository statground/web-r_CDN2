(function() {
  ...
  function StorySection() {
    return h("section", { id: "intro-story", className: "scroll-mt-24 border-t border-gray-200 bg-white px-5 py-16 md:px-8 md:py-20" }, h("div", { className: "mx-auto max-w-5xl" }, h(SectionHeader, { kicker: "Story", title: "통계마당의 스토리", description: "2013년 4월의 커뮤니티에서 시작해, 지금의 통계·데이터 서비스 회사로 확장되어 온 이야기입니다." }), h("div", { className: "mt-12 space-y-8" }, storyBlocks.map((block) => h("article", { key: block.title, className: "rounded-lg border border-gray-200 bg-gray-50 p-6 md:p-8" }, h("h3", { className: "text-xl font-bold leading-8 text-gray-950 md:text-2xl" }, block.title), h("div", { className: "mt-5 space-y-4 text-base leading-8 text-gray-700" }, block.body.map((paragraph) => h("p", { key: paragraph }, paragraph))))))));
  }
  function PeopleSection() {
    return h("section", { id: "intro-people", className: "scroll-mt-24 bg-white px-5 py-16 md:px-8 md:py-20" }, h("div", { className: "mx-auto max-w-6xl" }, h(SectionHeader, { kicker: "People", title: "만든 사람들", description: "통계마당과 Web-R의 시작을 함께 만든 사람들입니다." }), h("div", { className: "mt-10 grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6" }, people.map((person) => h(PersonCard, { key: "people-" + person.name, person })))));
  }
  ...
  function LogoRailSection() {
    return h("section", { id: "intro-network", className: "scroll-mt-24 border-t border-gray-200 bg-gray-50 px-5 py-16 md:px-8 md:py-20" }, h("div", { className: "mx-auto max-w-7xl" }, h(SectionHeader, { kicker: "Network", title: "함께한 파트너와 클라이언트", description: "통계마당의 교육, 분석, 플랫폼 경험이 닿았던 기업과 기관입니다." }), h("div", { className: "webr-intro-logo-rail mt-10" }, h("div", { className: "webr-intro-logo-rail-track py-2", "aria-label": "파트너와 클라이언트 로고" }, scrollingLogoItems.map((item, index) => h(LogoRailCard, { key: "logo-rail-" + index + "-" + item.name, item }))))));
  }
  function IntroContentsNav() {
    const items = [
      { label: "회사 소개", href: "#intro-overview" },
      { label: "스토리", href: "#intro-story" },
      { label: "만든 사람들", href: "#intro-people" },
      { label: "파트너/클라이언트", href: "#intro-network" }
    ];
    const children = [];
    items.forEach((item, index) => {
      if (index > 0) children.push(h("span", { key: "divider-" + item.href, className: "text-gray-300" }, "/"));
      children.push(h("a", { key: item.href, href: item.href, className: "transition hover:text-sky-900" }, item.label));
    });
    return h("nav", { className: "mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-x-3 gap-y-2 text-sm font-bold text-sky-700 md:text-base", "aria-label": "회사 소개 목차" }, children);
  }
  function Div_main() {
    return h("main", { className: "w-full bg-white text-gray-950" }, h("section", { id: "intro-overview", className: "scroll-mt-24 px-5 pt-14 pb-12 md:px-8 md:pt-20" }, h("div", { className: "mx-auto max-w-6xl text-center" }, h(IntroContentsNav, null), h("h1", { className: "mt-5 text-4xl font-extrabold text-gray-950 md:text-6xl" }, "주식회사 통계마당"), h("p", { className: "mx-auto mt-5 max-w-3xl text-lg leading-8 text-gray-600" }, "통계마당은 2013년 4월 통계와 데이터를 공부하고 활용하는 사람들이 모인 커뮤니티로 시작했습니다. 2021년 12월 주식회사 통계마당으로 전환한 뒤에도 커뮤니티를 꾸준히 유지하며, 통계·데이터 교육, 분석, Web-R 같은 서비스를 개발해 나가고 있습니다."))), h(StorySection, null), h(PeopleSection, null), h(LogoRailSection, null));
  }
  function set_main() {
