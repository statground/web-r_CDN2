(function installWebRNotebookApp202607251120(window, document) {
"use strict";

if (window.__webrNotebookApp202607251120Installed) {
  return;
}
window.__webrNotebookApp202607251120Installed = true;
...
function Notebook() {
  ...
  const webrInstanceRef = useRef(null);
  const runtimeRetryingRef = useRef(false);
  ...
  useEffect(() => {
    ...
    async function initWebR() {
      try {
        runtimeRetryingRef.current = false;
        setShowOverlay(false);
        setRuntimeFailed(false);
        setReady(false);
        setOverlayText("Downloading WebR runtime...");
        if (typeof window.__webrLoadRuntime !== "function") {
          throw new Error("WEBR_RUNTIME_LOADER_UNAVAILABLE");
        }
        const WebRClass = await window.__webrLoadRuntime({
          timeoutMs: 45000,
          onStage: (stage) => {
            if (stage === "module_loading") {
              setStatus("Downloading WebR...");
              setOverlayText("WebR 모듈을 내려받고 있습니다. 편집기는 지금 사용할 수 있습니다.");
            }
          }
        });
        if (cancelled)
        ...
        webrInstanceRef.current = webr;
        setStatus("Starting WebR...");
        setOverlayText("WebR 분석 엔진을 시작하고 있습니다.");
        await Promise.race([
          webr.init(),
          new Promise((_, reject) => window.setTimeout(
            () => reject(new Error("WEBR_INIT_TIMEOUT")),
            60000
          ))
        ]);
        if (cancelled)
          return;
        setStatus("Configuring WebR...");
        setOverlayText("분석 환경과 기본 패키지를 설정하고 있습니다.");
...
") }
        ...
        const pendingRestore = pendingRuntimeRestoreRef.current;
        if (pendingRestore) {
          setStatus("Restoring Notebook...");
          setOverlayText("저장된 패키지와 파일을 복원하고 있습니다.");
          await restoreRuntimeState(pendingRestore, webr);
        ...
        console.warn("WebR runtime initialization failed.");
        if (!cancelled) {
          setStatus("Failed to start WebR");
          setOverlayText("분석 엔진을 준비하지 못했습니다. 네트워크를 확인한 뒤 다시 시도해 주세요.");
    ...
    return () => {
      cancelled = true;
      const runtime = webrInstanceRef.current;
      if (runtime && typeof runtime.close === "function") {
        ...
        if (webrInstanceRef.current === runtime) {
          webrInstanceRef.current = null;
        }
      }
    };
  }, [runtimeRetryKey]);
...
${varName}`;
  ...
  async function handleDataUpload(e) {
    ...
    (async () => {
      ...
      try {
        ...
        await webrInstance.FS.writeFile(uniqueName, uint8);
  ...
  async function handleRemoveDataFile(target) {
    ...
    if (!window.confirm(`Delete '${target.name}'?`))
      return;
    try {
      await webrInstance.FS.unlink(target.path);
    } catch (e) {
      console.warn(e);
    }
  ...
  async function restoreRuntimeState(restored, runtime = webrInstanceRef.current || webrInstance) {
    try {
      if (!runtime || !restored)
        return;
      ...
      for (const pkg of pkgs) {
        if (!pkg)
          continue;
        try {
          await runtime.installPackages([pkg]);
        } catch (e) {
      ...
      for (const f of files) {
        try {
          ...
          await runtime.FS.writeFile(f.name, uint8);
        } catch (e) {
  ...
  useEffect(() => {
    if (!webrInstanceRef.current && !webrInstance)
      return;
    ...
    pendingRuntimeRestoreRef.current = null;
    (async () => {
      await restoreRuntimeState(pending, webrInstanceRef.current || webrInstance);
    })();
  }, [webrInstance]);
  async function loadNotebookFromDB(nbid, { setStatusText = true } = {}) {
    ...
    try {
      ...
      const activeRuntime = webrInstanceRef.current || webrInstance;
      if (activeRuntime) {
        await restoreRuntimeState(restored, activeRuntime);
        pendingRuntimeRestoreRef.current = null;
      }
      if (setStatusText)
        setStatus("Loaded");
  ...
  ), !isViewMode && !ready && /* @__PURE__ */ React.createElement(
    "aside",
    {
      className: "fixed bottom-4 right-4 z-30 w-[min(24rem,calc(100vw-2rem))] rounded-2xl border border-amber-200 bg-white p-4 text-sm text-slate-800 shadow-xl",
      role: runtimeFailed ? "alert" : "status",
      "aria-live": "polite",
      "aria-label": "WebR 분석 엔진 상태"
    },
    /* @__PURE__ */ React.createElement("div", { className: "flex items-start gap-3" },
      !runtimeFailed && /* @__PURE__ */ React.createElement("div", { className: "mt-0.5 h-5 w-5 flex-none animate-spin rounded-full border-2 border-amber-500 border-t-transparent", "aria-hidden": "true" }),
      /* @__PURE__ */ React.createElement("div", { className: "min-w-0 flex-1" },
        /* @__PURE__ */ React.createElement("div", { className: "font-semibold" }, runtimeFailed ? "분석 엔진 준비 실패" : "분석 엔진 준비 중"),
        /* @__PURE__ */ React.createElement("p", { className: "mt-1 text-xs leading-5 text-slate-600" }, overlayText),
        runtimeFailed && /* @__PURE__ */ React.createElement(
          "button",
          {
            type: "button",
            className: "mt-3 rounded-lg bg-orange-600 px-3 py-2 text-xs font-semibold text-white hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2",
            onClick: async () => {
              if (runtimeRetryingRef.current) return;
              runtimeRetryingRef.current = true;
              const runtime = webrInstanceRef.current;
              try {
              ...
              webrInstanceRef.current = null;
              setWebrInstance(null);
              if (typeof window.__webrResetRuntimeLoader === "function") {
                window.__webrResetRuntimeLoader();
              }
