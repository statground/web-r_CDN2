...
function sleepCommunitySidebar(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
function isCommunitySidebarPending(data) {
...
function communitySidebarRows(data, limit) {
  ...
  const rows = Object.values(data || {}).filter((item) => item && typeof item === "object" && item.pending !== true);
...
function createCommentEditorFallbackInHost(host, commentId, initialHTML = "") {
  ...
  const existing = findCommentFallbackTextarea(host, commentId);
  if (existing) {
    if (!existing.value && initialHTML) {
      existing.value = initialHTML;
    }
    return createArticleEditorFallback(existing);
  }
  host.classList.add("webr-comment-editor-host");
  const textarea = document.createElement("textarea");
  textarea.id = "txt_content_comment_" + commentId + "_fallback";
  textarea.name = "txt_content_comment_" + commentId + "_fallback";
...
function findCommentFallbackTextarea(host, commentId = null) {
  if (!host) {
    return null;
  }
  ...
  return textareas.find((textarea) => {
    if (textarea.getAttribute("data-comment-editor-fallback") === "1") {
      return true;
    }
    if (!key) {
      return false;
    }
    return textarea.parentElement === host && (textarea.id === "txt_content_comment_" + key || textarea.id === "txt_content_comment_" + key + "_fallback");
  }) || null;
}
function getCommentSolidSlot(host, commentId) {
  if (!host) {
    return null;
  }
  const key = commentId == null ? "new" : String(commentId);
  let slot = Array.from(host.children).find((child) => child && child.getAttribute && child.getAttribute("data-comment-editor-solid-slot") === key);
  if (!slot) {
    slot = document.createElement("div");
    slot.className = "w-full";
    slot.setAttribute("data-comment-editor-solid-slot", key);
    host.appendChild(slot);
  }
  return slot;
}
function commentEditorHostHasSolidUI(host) {
  if (!host) {
    return false;
  }
  const candidates = Array.from(host.querySelectorAll(".lre-root, .lre-app, .solid-embedded-editor-host, .lre-editor, [contenteditable='true'], .ProseMirror"));
...
function promoteSolidCommentEditor(commentId, host, solidHost, mounted) {
  const key = commentId == null ? "new" : String(commentId);
  if (!host || !solidHost || !mounted || mounted.failed || !commentEditorHostHasSolidUI(solidHost)) {
    return false;
  }
  solidHost.classList.forEach((className) => host.classList.add(className));
  host.classList.add("webr-comment-editor-host");
  const fallback = findCommentFallbackTextarea(host, key);
  if (fallback && fallback.parentNode) {
    const fallbackHTML = fallback.value || "";
    if (fallbackHTML && window.WebRSolidEditor && typeof window.WebRSolidEditor.setHTML === "function") {
      window.WebRSolidEditor.setHTML(mounted, fallbackHTML);
    }
    fallback.parentNode.removeChild(fallback);
  }
  communityState.commentEditors[key] = mounted;
  return true;
}
function scheduleSolidCommentEditorPromotion(commentId, host, solidHost, mounted) {
  const promote = () => promoteSolidCommentEditor(commentId, host, solidHost, mounted);
  if (typeof requestAnimationFrame === "function") {
    requestAnimationFrame(promote);
  }
  [80, 200, 500, 1000, 1800].forEach((delay) => setTimeout(promote, delay));
}
...
function ensureCommentEditorInput(commentId, hostID = null, initialHTML = "") {
  ...
  if (commentEditorHostHasSolidUI(host)) {
...
async function mountSolidCommentEditor(commentId, initialHTML = "", hostID = null) {
  ...
  const fallbackEditor = ensureCommentEditorInput(key, hostID, initialHTML || "");
  if (window.WebRSolidEditor && typeof window.WebRSolidEditor.mountHost === "function") {
    const solidHost = getCommentSolidSlot(host, key);
    try {
      const mounted = await window.WebRSolidEditor.mountHost(solidHost, editorOptions);
      if (mounted && !mounted.failed && promoteSolidCommentEditor(key, host, solidHost, mounted)) {
        scheduleCommentEditorRecovery(key, hostID, initialHTML || "");
        return mounted;
      }
      if (mounted && !mounted.failed) {
        scheduleSolidCommentEditorPromotion(key, host, solidHost, mounted);
      } else if (mounted && mounted.failed && solidHost.parentNode) {
        solidHost.parentNode.removeChild(solidHost);
      }
    } catch (error) {
      console.error("[mountSolidCommentEditor] solid editor mount failed", error);
      if (solidHost && solidHost.parentNode) {
        solidHost.parentNode.removeChild(solidHost);
      }
    }
  }
  scheduleCommentEditorRecovery(key, hostID, initialHTML || "");
  return fallbackEditor || ensureCommentEditorInput(key, hostID, initialHTML || "") || createCommentEditorFallbackInHost(host, key, initialHTML || "");
}
...
const COMMUNITY_SIDEBAR_RETRY_DELAY_MS = 800;
...
function pruneDeletedCommentTombstones(now = Date.now()) {
  const tombstones = communityState.deletedCommentIDs || {};
  Object.keys(tombstones).forEach((uuid) => {
    const age = now - Number(tombstones[uuid] || 0);
    if (!Number.isFinite(age) || age < 0 || age > COMMUNITY_COMMENT_DELETE_TOMBSTONE_MS) {
      delete tombstones[uuid];
    }
  });
  communityState.deletedCommentIDs = tombstones;
  return tombstones;
}
function markDeletedCommentUUIDs(uuids) {
  const now = Date.now();
  const tombstones = pruneDeletedCommentTombstones(now);
  (uuids || []).forEach((uuid) => {
    ...
    tombstones[key] = now;
    delete communityState.commentEditors[key];
    delete communityState.commentFiles[key];
  });
}
function isCommentDeletedLocally(uuid) {
  const key = String(uuid || "").trim();
  if (!key) {
    return false;
  }
  return Object.prototype.hasOwnProperty.call(pruneDeletedCommentTombstones(), key);
}
function isActiveComment(comment) {
  return !!comment && Number(comment.active == null ? 1 : comment.active) === 1;
}
function isDeletedCommentShell(comment) {
  if (!comment) {
    return false;
  }
  ...
  return Number(comment.is_deleted_shell || 0) === 1 || Number(comment.visible || 0) === 1 || comment.__optimisticAction === "delete-shell" || comment.__syntheticDeletedShell === true;
}
function deletedCommentShellRow(uuid, seed) {
  const row = seed || {};
  return {
    ...row,
    uuid,
...
function ensureDeletedParentShells(comments) {
  const rows = (comments || []).filter(Boolean);
  const byID = new Map(rows.map((item) => [item.uuid, item]));
  rows.forEach((comment) => {
    const parentID = String(comment && comment.uuid_upper || "").trim();
    if (!parentID || byID.has(parentID) || !isActiveComment(comment) || isCommentDeletedLocally(parentID)) {
      return;
    }
    const shell = deletedCommentShellRow(parentID, {
      uuid_article: comment.uuid_article,
      uuid_upper: "",
      ...
      user_writer: 0,
      rereply: {},
      __syntheticDeletedShell: true
    });
    rows.push(shell);
    byID.set(parentID, shell);
  });
  return rows;
}
function countActiveCommentRows(comment) {
  if (!comment) {
    return 0;
  }
  const replyCount = Object.values(comment.rereply || {}).reduce((count, reply) => count + countActiveCommentRows(reply), 0);
  return (isActiveComment(comment) ? 1 : 0) + replyCount;
}
function collectCommentTreeUUIDs(uuid_comment) {
...
function CommunityArticlePendingNotice(props) {
  return /* @__PURE__ */ React.createElement(
    "div",
    {
      class: "flex min-h-[160px] w-full items-center justify-center rounded-lg border border-amber-200 bg-amber-50 px-4 text-center text-sm font-medium text-amber-800",
      role: "status"
    },
...
function Div_article_read_comment(props) {
  ...
  function Div_comment(propsComment) {
    ...
    const deletedShell = isDeletedCommentShell(propsComment.data);
    const bgColorClass = deletedShell ? "bg-slate-50 border border-dashed border-slate-200" : propsComment.data.user_writer == 1 ? isDepth2 ? "bg-blue-100 border border-blue-700" : "bg-blue-50" : isDepth2 ? "bg-gray-50" : "bg-white";
...
async function click_btn_reply_comment(uuid_comment) {
  ...
  const replyForm = document.querySelector("#div_community_read_comment_new_" + uuid_comment + "_form");
  if (replyForm && !commentEditorHostHasSolidUI(replyForm)) {
    communityState.commentEditors[uuid_comment] = await mountSolidCommentEditor(uuid_comment, "");
    setCommentEditorHTML(communityState.commentEditors[uuid_comment], "");
  }
}
async function click_btn_edit_comment(uuid_comment) {
  function Div_comment_editor_form(props) {
    return /* @__PURE__ */ React.createElement("div", { class: "w-full rounded-xl border border-blue-200 bg-white p-3 shadow-sm" }, /* @__PURE__ */ React.createElement("div", { class: "w-full", id: "div_comment_editor_main_" + props.uuid_comment }), /* @__PURE__ */ React.createElement("div", { class: "w-full mt-2", id: "div_comment_edit_file_control_" + props.uuid_comment }, /* @__PURE__ */ React.createElement(AttachmentDropZone, { target: "comment", commentId: props.uuid_comment, compact: true })), /* @__PURE__ */ React.createElement("div", { class: "flex flex-row justify-end items-center w-full space-x-2 mt-2" }, /* @__PURE__ */ React.createElement(
      "input",
      {
...
async function removeCommentTreeFromView(uuid_comment) {
  const rows = Object.values(communityState.commentData || {}).filter(Boolean);
  const activeChildren = rows.filter((comment) => comment && comment.uuid_upper === uuid_comment && isActiveComment(comment) && !isCommentDeletedLocally(comment.uuid));
  if (activeChildren.length > 0) {
    let found = false;
    const nextRows = rows.map((comment) => {
      if (!comment || comment.uuid !== uuid_comment) {
        return comment;
      }
      found = true;
      return deletedCommentShellRow(uuid_comment, comment);
    });
    if (!found) {
      nextRows.push(deletedCommentShellRow(uuid_comment, {
        uuid_article: activeChildren[0].uuid_article,
        uuid_upper: "",
        created_at: activeChildren[0].created_at,
        updated_at: activeChildren[0].updated_at,
        user_nickname: "",
        user_role: "",
        user_writer: 0,
        rereply: {}
    ...
    communityState.commentData = indexedRowsFromArray(nextRows);
    await set_comment();
    return;
  }
  markDeletedCommentUUIDs([uuid_comment]);
  communityState.commentData = indexedRowsFromArray(rows.filter((comment) => comment && comment.uuid !== uuid_comment));
  await set_comment();
}
async function upsertCommentFromMutationResponse(comment, action = "submit") {
  if (!comment || !comment.uuid) {
    return;
  }
  const existing = Object.values(communityState.commentData || {}).find((item) => item && item.uuid === comment.uuid);
  const rows = Object.values(communityState.commentData || {}).filter((item) => item && item.uuid !== comment.uuid);
  const responseAttachments = normalizeAttachmentList(comment);
  const existingAttachments = normalizeAttachmentList(existing);
  rows.push({
    active: 1,
    visible: 1,
    ...(existing || {}),
...
function mergeOptimisticComments(responseData) {
  const rawServerRows = Object.values(responseData || {}).filter(Boolean);
  ...
  const optimisticRows = Object.values(communityState.commentData || {}).filter((item) => {
    ...
    if (rawServerIDs.has(item.uuid) && item.__optimisticAction !== "edit" && item.__optimisticAction !== "delete-shell") {
      return false;
    }
  ...
  const appendOptimisticRows = optimisticRows.filter((item) => !serverIDs.has(item.uuid));
  if (appendOptimisticRows.length === 0 && serverRows.length === rawServerRows.length && optimisticRows.every((item) => item.__optimisticAction !== "edit" && item.__optimisticAction !== "delete-shell")) {
    return responseData || {};
  }
  return indexedRowsFromArray(serverRows.concat(appendOptimisticRows));
}
...
async function set_comment() {
  ...
  const rawRows = Object.values(communityState.commentData).filter((c) => c && !isCommentDeletedLocally(c.uuid) && (isActiveComment(c) || isDeletedCommentShell(c)));
  const activeReplyParentIDs = new Set(rawRows.filter((c) => isActiveComment(c) && String(c.uuid_upper || "").trim()).map((c) => String(c.uuid_upper || "").trim()));
  const visibleRows = rawRows.filter((c) => isActiveComment(c) || activeReplyParentIDs.has(String(c.uuid || "").trim()));
  const allComments = ensureDeletedParentShells(visibleRows);
  ...
  const list_comment = communityState.commentUpper.map((comment) => ({
    ...comment,
    rereply: allComments.filter((item) => item.uuid_upper === comment.uuid && isActiveComment(item))
  }));
  ...
  allComments.forEach((comment) => {
    if (!comment || !comment.uuid)
      return;
    if (isDeletedCommentShell(comment))
      return;
