...
const myInfoStatementSignatureFallbackURL = "/account/myinfo/payment_statement_signature.png";
...
async function myInfoLoadStatementSignature(dataURL, signatureURL) {
  const embedded = myInfoText(dataURL).trim();
  const globals = myInfoGlobals();
  const candidates = [
    signatureURL,
    globals.payment_statement_signature_url,
    globals.statement_signature_url,
    embedded,
    myInfoStatementSignatureFallbackURL
  ];
  const seen = /* @__PURE__ */ new Set();
  for (const candidate of candidates) {
    const source = myInfoText(candidate).trim();
    if (!source || seen.has(source))
      continue;
    seen.add(source);
    if (source.startsWith("data:image/")) {
      const image = await myInfoImageFromSource(source);
      if (image)
        return image;
      continue;
    }
    const isFallback = source === myInfoStatementSignatureFallbackURL;
    const requestURL = isFallback ? `${source}?v=${Date.now()}` : source;
    try {
      const res = await fetch(requestURL, {
        credentials: "same-origin",
        cache: isFallback ? "no-store" : "force-cache"
      });
      if (!res.ok)
        continue;
      const image = await myInfoImageFromBlob(await res.blob());
      if (image)
        return image;
    } catch (_) {
    }
...
async function myInfoDownloadPaymentStatement(row, user, signatureDataURL, signatureURL) {
  const signatureImage = await myInfoLoadStatementSignature(signatureDataURL, signatureURL);
  if (!signatureImage) {
...
function MyInfoChart(props) {
  ...
  const [echartsTick, setEchartsTick] = React.useState(() => window.echarts ? 1 : 0);
  React.useEffect(() => {
    if (window.echarts) {
      setEchartsTick((value) => value || 1);
      return void 0;
    }
    let canceled = false;
    let retryTimer = 0;
    ...
    const notifyReady = () => {
      if (!canceled && window.echarts) {
        setEchartsTick((value) => value + 1);
      }
    };
    const loadFallback = () => {
      if (canceled || window.echarts || document.getElementById("myinfo-echarts-fallback")) {
        notifyReady();
        return;
      }
      fallbackScript = document.createElement("script");
      fallbackScript.id = "myinfo-echarts-fallback";
      fallbackScript.src = "https://cdn.jsdelivr.net/npm/echarts@6.0.0/dist/echarts.min.js";
      fallbackScript.defer = true;
      fallbackScript.onload = notifyReady;
      document.head.appendChild(fallbackScript);
    };
    const existing = Array.from(document.scripts || []).find((script) => myInfoText(script.src).includes("echarts"));
    if (existing) {
      existing.addEventListener("load", notifyReady, { once: true });
      existing.addEventListener("error", loadFallback, { once: true });
    } else {
      loadFallback();
    }
    let retries = 0;
    const poll = () => {
      ...
      retryTimer = window.setTimeout(poll, 125);
    };
    poll();
    return () => {
      canceled = true;
      ...
      if (existing) {
        existing.removeEventListener("load", notifyReady);
        existing.removeEventListener("error", loadFallback);
      }
      if (fallbackScript) {
        fallbackScript.onload = null;
      }
    };
  }, []);
  React.useEffect(() => {
    ...
    const initChart = () => {
      ...
      if (width < 16 || height < 16) {
        retryTimer = window.setTimeout(initChart, 80);
        return;
      }
...
function MyInfoComments(props) {
  ...
  const showChart = !!chartOption && rows.length > 1;
  return /* @__PURE__ */ React.createElement(MyInfoPanel, { title: "\uB0B4\uAC00 \uC4F4 \uB313\uAE00" }, props.loading ? /* @__PURE__ */ React.createElement(MyInfoChartSkeleton, { className: "h-[220px] w-full" }) : rows.length === 0 ? /* @__PURE__ */ React.createElement(MyInfoTableEmpty, null, "\uC791\uC131\uD55C \uB313\uAE00\uC774 \uC5C6\uC2B5\uB2C8\uB2E4.") : /* @__PURE__ */ React.createElement("div", null, showChart ? /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement(MyInfoChart, { option: chartOption, className: "h-[220px] w-full", empty: "\uB313\uAE00 \uC791\uC131 \uCE98\uB9B0\uB354\uB97C \uD45C\uC2DC\uD560 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4." })) : null, /* @__PURE__ */ React.createElement("div", { className: "space-y-3" }, rows.map((row) => /* @__PURE__ */ React.createElement("div", { key: myInfoText(row.uuid), className: "rounded-lg border border-slate-200 bg-white px-5 py-4 hover:border-sky-300 hover:bg-sky-50" }, /* @__PURE__ */ React.createElement("div", { className: "flex flex-row items-center justify-between gap-2 sm:flex-col sm:items-start" }, /* @__PURE__ */ React.createElement("a", { className: "font-semibold text-slate-950 hover:text-sky-700", href: myInfoCommentHref(row) }, myInfoText(row.article_title) || "\uAC8C\uC2DC\uAE00"), /* @__PURE__ */ React.createElement("div", { className: "text-sm text-slate-500" }, myInfoDate(row.created_at))), /* @__PURE__ */ React.createElement(MyInfoHTMLPreview, { html: row.content }))))));
}
              ...
              React.createElement(
                ...
                React.createElement(
                  "button",
                  {
                    ...
                    onClick: () => myInfoDownloadPaymentStatement(row, props.user || {}, signatureDataURL, signatureURL)
                  },
                  "\uAC70\uB798\uBA85\uC138\uC11C"
                )
              )
...
function myInfoSectionDataKeys(section) {
  if (section === "overview")
    return ["articles", "comments", "payments"];
  if (["articles", "comments", "payments", "connection"].includes(section))
    return [section];
  return [];
}
function MyInfoApp() {
  ...
  React.useEffect(() => {
    ...
    const onHash = () => {
      ...
      if (legacyURL2)
        window.location.replace(legacyURL2);
    };
