(function installWebRProductHeader202607251149(window, document) {
"use strict";

if (window.__webrProductHeader202607251149Installed) {
  return;
}
window.__webrProductHeader202607251149Installed = true;
...
function currentWebRCDN2Base() {
  ...
  const scriptURL = current
    ? (current.getAttribute && current.getAttribute("data-webr-cdn-original")) || current.src || ""
    : "";
...
const MENU_SECTIONS = {
  webr: {
    ...
    items: [
      { href: "/webr/roc-analysis/sample/", title: "샘플 ROC 분석", description: "로그인 없이 예제 데이터를 실행하고 결과 형식을 확인합니다.", icon: "webr2", image: WEBR_CDN2 + "images/svg/R_Logo_20.svg" },
      ...
      { href: "/webr/", title: "Web-R Classic", description: "기존 무료 Web-R 분석 환경으로 이동합니다.", icon: "r-logo-dark", image: WEBR_CDN2 + "images/svg/R_logo_black.svg" },
  ...
  r_ecosystem: {
    ...
    items: [
      { href: "/r-ecosystem/", title: "R 소식·글", description: "공식 발표, 블로그·해설, 저널·뉴스레터를 모아 봅니다.", icon: "notice", image: WEBR_CDN2 + "images/svg/menu_notice.svg" },
      ...
      { href: "/community/", title: "커뮤니티", description: "Web-R 이용자 게시판과 외부 R 커뮤니티 글을 봅니다.", icon: "community", image: WEBR_CDN2 + "images/svg/menu_free.svg" },
  ...
  book: {
    ...
    items: [
      { href: "/book/001/", title: "웹에서 클릭만으로 하는 R통계분석", description: "Web-R 기반 통계분석 실습 도서입니다.", icon: "book", image: WEBR_CDN + "images/book/book_003.jpg" },
      { href: "/book/002/", title: "Learning ggplot2 Using Shiny App", description: "ggplot2 학습과 Shiny 예제를 함께 봅니다.", icon: "book", image: WEBR_CDN + "images/book/book_004.jpg" },
      { href: "/book/003/", title: "R을 이용한 조건부과정분석", description: "조건부과정분석 예제와 보조 자료입니다.", icon: "book", image: WEBR_CDN + "images/book/book_002.jpg" },
      { href: "/book/004/", title: "의학논문 작성을 위한 R통계와 그래프", description: "의학 논문 작성과 R 그래프 실습 자료입니다.", icon: "book", image: WEBR_CDN + "images/book/book_001.jpg" },
      { href: "/book/005/", title: "밑바닥부터 시작하는 ROC 커브 분석", description: "ROC 커브 분석 실습 자료입니다.", icon: "book", image: WEBR_CDN + "images/book/book_006.jpg" },
      { href: "/book/006/", title: "의료인을 위한 R 생존분석", description: "의료 연구 생존분석 실습 자료입니다.", icon: "book", image: WEBR_CDN + "images/book/book_008.jpg" },
      { href: "/book/007/", title: "웹R을 이용한 통계분석", description: "웹R 기반 통계분석 예제입니다.", icon: "book", image: WEBR_CDN + "images/book/book_007.jpg" },
      { href: "/book/008/", title: "일반화가법모형 소개", description: "GAM 분석 예제와 설명 자료입니다.", icon: "book", image: WEBR_CDN + "images/book/book_005.jpg" }
    ]
  },
  workshop: {
    title: "워크샵",
    ...
    items: [
      { href: "/workshop/", title: "워크샵", description: "워크샵 안내와 강의 자료로 이동합니다.", icon: "workshop", image: WEBR_CDN2 + "images/svg/menu_workshop.svg" },
      { href: "/workshop/lecture/", title: "강의", description: "R 관련 Inflearn 강의를 탐색합니다.", icon: "lecture", image: WEBR_CDN2 + "images/svg/lecturer.svg" },
...
const MENUS = ["webr", "r_ecosystem", "intro"];
const MenuState = {
  hamburger: false,
...
function closeAllMenus() {
  ...
  applyMenuStateClasses();
}

function applyMenuStateClasses() {
  MENUS.forEach(function(menu) {
    const pc = document.getElementById("div_megamenu_" + menu);
    const mobile = document.getElementById("div_menu_mobile_" + menu);
    const trigger = document.getElementById("btn_menu_desktop_" + menu);
    const willOpen = !!MenuState.sections[menu];
...
function click_dropdown(id) {
  ...
  const willOpenBook = id === "book" && !MenuState.sections.book;
  if (willOpenBook && !BookMenuState.ready) {
    ensureBookMenuItems().then(function(changed) {
      if (changed) {
        renderMenuShell();
      }
      click_dropdown(id);
...
function click_hamburger() {
  ...
  const trigger = document.getElementById("btn_menu_hamburger");
  if (trigger) trigger.setAttribute("aria-expanded", MenuState.hamburger ? "true" : "false");
}
...
function MenuIcon(props) {
  ...
  if (icon === "lecture") {
    return h("svg", base,
      h("path", Object.assign({}, stroke, { d: "M5 4h14v16H5zM8 8h8M8 12h5" })),
      h("path", Object.assign({}, stroke, { d: "M15 14l3 2-3 2v-4z" }))
    );
  }
  if (icon === "youtube") {
    return h("svg", base,
...
function renderMenuShell() {
  ...
  if (menu && typeof Div_menu === "function") {
    ReactDOM.render(h(Div_menu), menu);
    applyMenuStateClasses();
  }
}

function normalizeBookMenuItem(item) {
  item = item || {};
  const href = String(item.href || "").trim();
  const sub = String(item.sub || item.board_url_sub || item.uuid_board_category || href.replace(/^\/book\//, "").replace(/\/$/, "")).trim();
  const title = String(item.title || "").trim();
  if (!sub || !title) {
    return null;
  }
  const description = String(item.description || [item.publisher, item.published_at].filter(Boolean).join(" · ") || "R 도서 상세와 관련 글을 확인합니다.").trim();
  return {
    href: href || "/book/" + sub + "/",
    title,
    description,
    icon: "book",
    image: String(item.image || item.url_image || "").trim()
  };
}

function applyBookMenuItems(items) {
  const normalized = (items || []).map(normalizeBookMenuItem).filter(Boolean);
  if (!normalized.length) {
    return false;
  }
  normalized.sort(function(a, b) {
    return a.href.localeCompare(b.href, "en", { numeric: true, sensitivity: "base" });
  });
  MENU_SECTIONS.book.items = normalized;
  return true;
}

function ensureBookMenuItems() {
  if (BookMenuState.ready) {
    return Promise.resolve(false);
  }
  if (BookMenuState.promise) {
    return BookMenuState.promise;
  }
  BookMenuState.loading = true;
  BookMenuState.promise = fetch("/ajax_book_menu/", { method: "GET", credentials: "same-origin" })
    .then(function(res) {
      if (!res.ok) throw new Error("HTTP " + res.status);
      return res.json();
    })
    .then(function(data) {
      return applyBookMenuItems(data && data.items);
    })
    .catch(function(err) {
      console.warn("[WebRMenu] book menu fallback is used:", err);
      return false;
    })
    .finally(function() {
      BookMenuState.ready = true;
      BookMenuState.loading = false;
      BookMenuState.promise = null;
    });
  return BookMenuState.promise;
}
...
function ImageMegaMenu(props) {
  ...
  const columns = splitColumns(section.items, props.id === "workshop" ? 3 : hasFeature ? 3 : 4);
  const gridClass = props.id === "workshop"
    ? "mx-auto grid max-w-5xl grid-cols-1 gap-6 px-6 py-4 text-sm text-gray-600 md:grid-cols-3 md:px-10"
...
function LegacyIntroMenu() {
  ...
  return h("nav", { id: "div_megamenu_intro", className: "hidden", "aria-label": "Web-R 소개 하위 메뉴" },
    h("div", { className: "mx-auto grid max-w-screen-xl grid-cols-1 gap-6 px-4 py-4 text-sm text-gray-600 md:grid-cols-3 md:px-6" },
...
function DesktopNavItem(props) {
  ...
  return h("button", {
    id: "btn_menu_desktop_" + props.id,
    type: "button",
    className: "inline-flex cursor-pointer items-center whitespace-nowrap bg-transparent px-2 py-2 text-sm font-medium text-gray-900 hover:text-blue-700",
    onClick: function() { click_dropdown(props.id); },
    "aria-controls": "div_megamenu_" + props.id,
    "aria-expanded": MenuState.sections[props.id] ? "true" : "false"
...
function MobileMenuItem(props) {
  ...
  return h("div", { className: "w-full" },
    h("button", {
      ...
      onClick: function() {
        if (props.id === "book" && !open && !BookMenuState.ready) {
          ensureBookMenuItems().then(function() {
            setOpen(true);
          });
          return;
        }
...
window.get_menu_header = get_menu_header;
window.click_dropdown = click_dropdown;
})(window, document);
