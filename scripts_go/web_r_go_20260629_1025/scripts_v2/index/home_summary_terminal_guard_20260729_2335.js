(function installWebRHomeSummaryTerminalGuard202607292335(window, document) {
  "use strict";

  if (window.__webrHomeSummaryTerminalGuard202607292335Installed) {
    return;
  }
  window.__webrHomeSummaryTerminalGuard202607292335Installed = true;
  ...
  var summaryStatisticKeys = ["cnt_member", "cnt_visitor", "cnt_pageview"];
  ...
  function normalizeSummary(payload, requireComplete) {
    if (
      ...
      (requireComplete && payload.complete !== true)
    ) {
      return null;
    }
    var normalized = {
    ...
    summaryStatisticKeys.forEach(function copyAllowedStatistic(key) {
      var value = Number(payload.statistics[key]);
      normalized.statistics[key] = Number.isFinite(value) && value >= 0
        ? Math.floor(value)
        : 0;
    });
    return normalized;
  ...
  function readCachedSummary() {
    try {
      ...
      return normalizeSummary(entry.payload, true);
    } catch (error) {
      return null;
    }
  }
  ...
  function mergeWithCachedSummary(livePayload) {
    ...
    summaryStatisticKeys.forEach(function retainLastGoodStatistic(key) {
      if (!Number.isFinite(Number(livePayload.statistics[key]))) {
        live.statistics[key] = cachedSummary.statistics[key];
      }
    });
    return live;
  }
    ...
    return {
      ...
      json: function normalizedSummaryJSON() {
        return response.json().then(function inspectPublicSummary(payload) {
          var normalized = normalizeSummary(payload, false);
          if (!normalized) {
            return payload;
          }
          ...
          return mergeWithCachedSummary(normalized);
        });
      }
    };
  }
