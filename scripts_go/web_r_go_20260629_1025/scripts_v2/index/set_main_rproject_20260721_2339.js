...
const WEBR_HOME_PAYLOAD_CACHE_PREFIX = "webr.home.payload.v3.";
// A complete payload is only used after the network response fails its
// freshness/shape check, so retain it long enough to cover a multi-hour DB or
// CDN incident without freezing normal successful responses.
const WEBR_HOME_PAYLOAD_CACHE_TTL = 7 * 24 * 60 * 60 * 1e3;
...
function homePostJSON(url, timeoutMs) {
  ...
  if (cached) {
    fetchJSON(firstWaitMs).then((value) => {
...
function Div_home_update_dashboard() {
  ...
  React.useEffect(() => {
    ...
    let boardGraceTimer = setTimeout(() => {
      if (!mounted)
        return;
      setHomeState((prev) => Object.assign({}, prev, {
        boardRecovering: true,
        loading: Object.assign({}, prev.loading, { board: false })
      }));
    }, 8e3);
    ...
    function loadCompleteBoard() {
      homePostJSON("/ajax_index_board/").then((board) => {
        if (!mounted)
          return;
        if (!homePayloadLooksUsable("/ajax_index_board/", board)) {
          boardRetryTimer = setTimeout(loadCompleteBoard, 15e3);
          return;
        }
        if (boardGraceTimer) {
          clearTimeout(boardGraceTimer);
          boardGraceTimer = null;
        }
        finishSlice("board", {
  ...
  const categoryDefs = [
    ...
    { id: "rcommunity", label: "R Community", href: "/community/", tone: "border-blue-200 bg-blue-50 text-blue-700", loading: homeState.loading.board, recovering: homeState.boardRecovering },
    ...
    { id: "community", label: "커뮤니티", href: "/community/", tone: "border-sky-200 bg-sky-50 text-sky-700", loading: homeState.loading.board, recovering: homeState.boardRecovering },
