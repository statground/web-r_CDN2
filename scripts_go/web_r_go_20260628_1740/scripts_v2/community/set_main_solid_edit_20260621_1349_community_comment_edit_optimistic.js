...
async function removeCommentTreeFromView(uuid_comment) {
  const removeIDs = collectCommentTreeUUIDs(uuid_comment);
  markDeletedCommentUUIDs(Array.from(removeIDs));
...
function mergeOptimisticComments(responseData) {
  ...
  const optimisticRows = Object.values(communityState.commentData || {}).filter((item) => {
    ...
    if (rawServerIDs.has(item.uuid) && item.__optimisticAction !== "edit") {
      return false;
    }
  ...
  const serverRows = rawServerRows.filter((item) => item && !isCommentDeletedLocally(item.uuid)).map((item) => {
    ...
    if (optimistic && optimistic.__optimisticAction === "edit") {
      return optimistic;
    }
    return item;
  });
  ...
  if (appendOptimisticRows.length === 0 && serverRows.length === rawServerRows.length && optimisticRows.every((item) => item.__optimisticAction !== "edit")) {
    return responseData || {};
  }
...
async function set_comment() {
  ...
  const allComments = Object.values(communityState.commentData).filter((c) => c && Number(c.active == null ? 1 : c.active) === 1 && !isCommentDeletedLocally(c.uuid));
