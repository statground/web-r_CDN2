...
function DivBoardTabs() {
  ...
  return /* @__PURE__ */ React.createElement("div", { class: "flex flex-wrap items-center gap-2 w-full pt-2" }, /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "all", onClick: () => handleChangeTab("all") }, "\uC804\uCCB4\uBCF4\uAE30"), /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "free", onClick: () => handleChangeTab("free") }, "\uC790\uC720\uAC8C\uC2DC\uD310"), /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "rcommunity", onClick: () => handleChangeTab("rcommunity") }, "R Community"), /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "notebook", onClick: () => handleChangeTab("notebook") }, "Web-R Notebook"), /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "mine", onClick: () => handleChangeTab("mine") }, "\uB0B4\uAC00 \uC4F4 \uAE00"), /* @__PURE__ */ React.createElement(TabButton, { active: activeTab === "commented", onClick: () => handleChangeTab("commented") }, "\uB0B4\uAC00 \uB313\uAE00\uC744 \uC4F4 \uAE00"));
}
...
function filterIndexedRows(data, searchText) {
  const needle = String(searchText || "").trim().toLowerCase();
  if (!needle) {
    return data || {};
  }
  return indexedRowsFromArray(Object.values(data || {}).filter((row) => {
    const haystack = [row.title, row.user_nickname, row.created_at, row.category, row.source_name].map((value) => String(value || "")).join(" ").toLowerCase();
    return haystack.includes(needle);
  }));
}
function commentsToCommentedArticleRows(data, searchText) {
  const needle = String(searchText || "").trim().toLowerCase();
  const rows = [];
  const seen = {};
  Object.values(data || {}).filter(Boolean).forEach((comment) => {
    ...
    const content = stripHTMLText(comment.content);
    const haystack = [title, content, comment.created_at].map((value) => String(value || "")).join(" ").toLowerCase();
...
async function get_article_list(loadMode, requestedPage = 1) {
  ...
  if (privateFilter) {
    ...
    const listData = activeTag === "mine" ? filterIndexedRows(privateData, searchText) : commentsToCommentedArticleRows(privateData, searchText);
