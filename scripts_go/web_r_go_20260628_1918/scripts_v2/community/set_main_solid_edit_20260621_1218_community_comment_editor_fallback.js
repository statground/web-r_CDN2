...
function commentEditorHostHasInput(host) {
  if (!host) {
    return false;
  }
  return !!host.querySelector("textarea, [contenteditable='true'], [role='textbox'], .ProseMirror");
}
...
function scheduleCommentEditorRecovery(commentId, hostID = null, initialHTML = "") {
  const key = commentId == null ? "new" : String(commentId);
  const recover = () => {
    const fallback = recoverCommentEditorIfMissing(key, hostID, initialHTML);
    if (fallback) {
...
async function mountSolidCommentEditor(commentId, initialHTML = "", hostID = null) {
  ...
  if (window.WebRSolidEditor && typeof window.WebRSolidEditor.mountHost === "function") {
    try {
      host.innerHTML = "";
      const mounted = await window.WebRSolidEditor.mountHost(host, editorOptions);
      scheduleCommentEditorRecovery(key, hostID, initialHTML || "");
      if (mounted && commentEditorHostHasInput(host)) {
        return mounted;
      }
    } catch (error) {
  ...
  return recoverCommentEditorIfMissing(key, hostID, initialHTML || "") || createCommentEditorFallbackInHost(host, key, initialHTML || "");
}
