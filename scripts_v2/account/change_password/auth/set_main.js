			...
			<div class="flex flex-col justify-start items-center w-[500px] p-[40px] space-y-[24px]
				...
				<div class="flex flex-col justify-center items-center text-start w-full">
					<svg aria-hidden="true"
						 class="w-8 h-8 text-gray-200 animate-spin fill-blue-600 mb-4" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div class="flex flex-col justify-center items-center w-full">
			<div class="flex flex-col justify-start items-center w-[500px] p-[40px] space-y-[24px]
				...
				<div class="flex flex-col justify-center items-center text-start w-full space-y-[12px]">
					...
					<div id="btn_submit" class="w-full">
						<Div_btn_submit class={class_btn_disabled} function={null} text={"비밀번호 변경"} />
					</div>
				</div>
...
function input_checker() {
	...
	if (txt_password.length < 8) {
		...
		ReactDOM.render(
			<Div_btn_submit class={class_btn_enabled} function={() => click_btn_submit()} text={"비밀번호 변경"} />,
			document.getElementById("btn_submit")
		)
	}
}
...
async function check_auth_code() {
	try {
		...
		ReactDOM.render(<Div_main_Error text={"인증 코드를 확인하는 중 오류가 발생했습니다."} />, document.getElementById("div_main"))
	}
}
...
async function click_btn_submit() {
	...
	ReactDOM.render(
		<Div_btn_submit_spinner class={class_btn_enabled + " cursor-not-allowed"} function={null} text={"비밀번호 변경"} />,
		document.getElementById("btn_submit")
	)
