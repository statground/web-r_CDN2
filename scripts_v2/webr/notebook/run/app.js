  ...
  async function loadNotebookFromDB(nbid, { setStatusText = true } = {}) {
    ...
    try {
      ...
      try {
        ...
        const _shareValNum = (String(_shareVal) === "2" || _shareVal === 2)
          ? 2
          : ((String(_shareVal) === "1" || _shareVal === 1 || _shareVal === true) ? 1 : 0);
        ...
        setShareUUID(_shareValNum > 0 ? String(_uuidShare || "") : "");
      } catch (e) {}
...
>
  ...
  {/* Package Manager */}
      ...
      {showLogin && (
        ...
        >
          <div className={"w-full max-w-md rounded-2xl border p-5 shadow-2xl " + (darkMode ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-200 bg-white text-slate-800")}>
            <div className="mb-2 flex items-center justify-between">
              <h2 className="text-sm font-semibold">Web-R 계정</h2>
            ...
            <div className="mb-3 flex gap-2 text-[12px]">
              <button type="button" onClick={() => { setLoginTab("signin"); setAuthError(""); }} className={"px-3 py-1 rounded-full border " + (loginTab === "signin" ? "bg-slate-200/20" : "")}>로그인</button>
              <button type="button" onClick={() => { setLoginTab("signup"); setAuthError(""); }} className={"px-3 py-1 rounded-full border " + (loginTab === "signup" ? "bg-slate-200/20" : "")}>회원가입</button>
              <button type="button" onClick={() => { setLoginTab("forgot"); setAuthError(""); setForgotStep(1); }} className={"px-3 py-1 rounded-full border " + (loginTab === "forgot" ? "bg-slate-200/20" : "")}>비밀번호 찾기</button>
            </div>

            {authError && (
              <div className="mb-3 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-[11px] text-red-700">
                {authError}
              </div>
            )}
            ...
            {loginTab === "signin" && (
              ...
              >
                <div className="space-y-1">
                  <label className="block text-[11px] text-slate-500">이메일(ID)</label>
                  <input type="text" value={loginAccount} onChange={(e) => setLoginAccount(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] text-slate-500">비밀번호</label>
                  <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                </div>
                <button type="submit" disabled={authWorking} className={"w-full rounded-xl px-3 py-2 text-[12px] font-semibold " + (authWorking ? "bg-slate-400" : "bg-orange-500 hover:bg-orange-600 text-white")}>
                  {authWorking ? "처리 중..." : "로그인"}
                </button>
              </form>
            )}
            ...
            {loginTab === "signup" && (
              ...
              >
                <div className="grid grid-cols-1 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[11px] text-slate-500">이메일</label>
                    <input type="text" value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-[11px] text-slate-500">비밀번호</label>
                    <input type="password" value={signupPassword} onChange={(e) => setSignupPassword(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-[11px] text-slate-500">닉네임</label>
                    <input type="text" value={signupNickname} onChange={(e) => setSignupNickname(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="block text-[11px] text-slate-500">이름</label>
                      <input type="text" value={signupRealname} onChange={(e) => setSignupRealname(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                    </div>
                    <div className="space-y-1">
                      <label className="block text-[11px] text-slate-500">성별</label>
                      <select value={signupGender} onChange={(e) => setSignupGender(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")}>
              ...
              <div className="space-y-3 text-[12px]">
                {forgotStep === 1 && (
                  <div className="space-y-3">
                    <div className="text-[11px] text-slate-400">가입한 이메일로 인증코드를 발송합니다.</div>
                    <div className="space-y-1">
                      <label className="block text-[11px] text-slate-500">이메일</label>
                      <input type="text" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                    </div>
                    <button
                      type="button"
                      disabled={authWorking}
                      onClick={async () => {
                        setAuthError("");
                        if (!forgotEmail) { setAuthError("이메일을 입력해주세요."); return; }
                        setAuthWorking(true);
                        try {
                ...
                {forgotStep === 2 && (
                  <div className="space-y-3">
                    <div className="text-[11px] text-slate-400">메일로 받은 인증코드를 입력해주세요.</div>
                    <div className="space-y-1">
                      <label className="block text-[11px] text-slate-500">인증코드</label>
                      <input type="text" value={forgotCode} onChange={(e) => setForgotCode(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                    </div>
                    <button
                      type="button"
                      disabled={authWorking}
                      onClick={async () => {
                        setAuthError("");
                        if (!forgotCode) { setAuthError("인증코드를 입력해주세요."); return; }
                        setAuthWorking(true);
                        try {
                          const r = await apiCheckAuthCode(forgotCode);
                          if (r && r.result === "ok") setForgotStep(3);
                          else setAuthError((r && (r.err || r.msg)) ? (r.err || r.msg) : "인증코드 확인에 실패했습니다.");
                        } catch (err) {
                    ...
                    <button type="button" className="w-full rounded-xl border px-3 py-2" onClick={() => setForgotStep(1)}>이메일 다시 입력</button>
                  </div>
                )}

                {forgotStep === 3 && (
                  <div className="space-y-3">
                    <div className="text-[11px] text-slate-400">새 비밀번호를 입력해주세요.</div>
                    <div className="space-y-1">
                      <label className="block text-[11px] text-slate-500">새 비밀번호</label>
                      <input type="password" value={forgotNewPw} onChange={(e) => setForgotNewPw(e.target.value)} className={"w-full rounded-xl border px-3 py-2 outline-none " + (darkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white")} />
                    </div>
                    <button
                      type="button"
                      disabled={authWorking}
                      onClick={async () => {
                        setAuthError("");
                        if (!forgotEmail || !forgotNewPw) { setAuthError("이메일과 새 비밀번호를 입력해주세요."); return; }
                        setAuthWorking(true);
                        try {
