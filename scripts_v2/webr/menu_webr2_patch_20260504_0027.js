(function () {
  ...
  function hasService(root) {
    return Array.from(root.querySelectorAll("[data-webr2-menu], a, div, span"))
      .some((node) => textOf(node) === serviceTitle || node.getAttribute("data-webr2-menu") === "true");
  }
  ...
  function findMemberNode(root) {
    return Array.from(root.querySelectorAll("ul, div, li, span")).find((node) => textOf(node) === "정회원 서버 접속")
      || Array.from(root.querySelectorAll("ul, div, li")).find((node) => textOf(node).includes("정회원 서버 접속"));
  }
  ...
  function hasWorkshopItem(root) {
    return Array.from(root.querySelectorAll("[data-workshop-menu], a, div, span"))
      .some((node) => {
        const href = node.getAttribute && node.getAttribute("href");
  ...
  function patchDesktopMenu() {
    ...
    if (hasService(menu)) return true;
    ...
    if (!memberList || !memberList.parentNode) return false;
  ...
  function patchMobileMenu() {
    ...
    if (!memberItem || !memberItem.parentNode) return false;
  ...
  function patchDesktopWorkshopMenu() {
    ...
    if (hasWorkshopItem(menu)) return true;
  ...
  function wrapDivMenu() {
    if (window.__webr2DivMenuWrapped) return;
    if (typeof Div_menu !== "function" || !window.React || !React.useEffect) return;
