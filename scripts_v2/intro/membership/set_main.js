...
// /intro/membership/ main + /intro/membership/result/ 를 하나의 set_main.js 로 통합
...
function membership_set_main_main() {
...
// 템플릿/URL 파라미터 병합
let membership_payment_key = "{{payment_key}}";
let membership_amount = "{{amount}}";
let membership_order_id = (typeof orderID !== "undefined" ? orderID : "{{orderID}}");
(() => {
  const url = new URL(window.location.href);
  const qp = (k) => url.searchParams.get(k);
  if (!membership_payment_key || membership_payment_key.includes("{{")) membership_payment_key = qp("paymentKey") || qp("payment_key") || "";
  if (!membership_amount || membership_amount.includes("{{")) membership_amount = qp("amount") || "";
  if (!membership_order_id || membership_order_id === "None" || membership_order_id.includes("{{")) membership_order_id = qp("orderId") || qp("orderID") || "";
})();
...
async function get_result_order_id() {
  const url = `/ajax_finish_order_id/?paymentKey=${encodeURIComponent(membership_payment_key)}&orderID=${encodeURIComponent(membership_order_id)}&amount=${encodeURIComponent(membership_amount)}`;
...
function membership_set_main_result() {
  // ✅ 중복 실행 방지
...
function set_main() {
    if (mode === "result") {
        return membership_set_main_result();
    }
    return membership_set_main_main();
}
