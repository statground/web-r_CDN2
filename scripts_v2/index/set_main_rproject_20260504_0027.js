...
async function get_div_main_board() {
    ...
    function Col(props) {
        ...
        const rprojectList = arr
            .filter((x) => x.category_url === "rproject")
            .sort(sortByCreatedAtDesc);
        ...
        if (rprojectList.length) pick.push(rprojectList[0]);
        ...
        if (activeTab === "rproject") current = rprojectList;
...
async function get_div_main_new_event() {
    const BLOCKED_EVENT_NICKNAMES = new Set(["탈퇴한 유저", "탈퇴한 회원", "Unknown", "unknown", "null", "None", "undefined"]);
    ...
    function isVisibleEventItem(item) {
        ...
        if (!nickname || BLOCKED_EVENT_NICKNAMES.has(nickname)) return false;
        return true;
    }
    ...
    function Col(props) {
        const sortedData = Object.values(props.data || {})
            .filter(isVisibleEventItem)
            .sort((a, b) => {
