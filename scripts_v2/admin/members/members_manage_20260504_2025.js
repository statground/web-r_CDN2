...
function AdminMembersManageMain(props) {
  ...
  async function deleteMember(member) {
    const uuid = adminMemberText(member.uuid);
    if (!uuid || !adminMemberIsBot(member)) return;
    if (!window.confirm("Bot 계정을 삭제 처리할까요? 작성자 이력을 보존하기 위해 비활성/차단 상태로 전환됩니다.")) return;
    ...
    try {
      ...
      const payload = await res.json();
      if (!payload || payload.ok === false) throw new Error((payload && payload.error) || "삭제 처리에 실패했습니다.");
      await reloadMembers();
      setEditingUUID("");
              ...
              {message ? <p className="text-sm font-medium text-blue-700">{message}</p> : null}
                ...
                <tbody>
                  ...
                  {pagedMembers.map((member, idx) => {
                    ...
                    const canDeleteBot = adminMemberIsBot(member)
                      && !deletedBot
                      && adminMemberNumber(member.is_staff) !== 1
                      && adminMemberNumber(member.is_superuser) !== 1;
                    return (
                    <React.Fragment key={member.uuid || member.email || idx}>
                      <tr className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-right whitespace-nowrap">
                          <button type="button" className="rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-50" disabled={deletingUUID === rowUUID} onClick={() => editingUUID === rowUUID ? setEditingUUID("") : beginEdit(member)}>
                            {editingUUID === rowUUID ? "닫기" : "편집"}
                          </button>
                          {canDeleteBot ? (
                            <button type="button" className="ml-2 rounded-md border border-rose-200 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50" disabled={deletingUUID === rowUUID} onClick={() => deleteMember(member)}>
                              {deletingUUID === rowUUID ? "삭제 중" : "삭제"}
                            </button>
                          ) : null}
                          {deletedBot ? <span className="ml-2 text-xs font-semibold text-slate-400">삭제됨</span> : null}
                        </td>
                      </tr>
                      {renderEditor(member)}
                    </React.Fragment>
